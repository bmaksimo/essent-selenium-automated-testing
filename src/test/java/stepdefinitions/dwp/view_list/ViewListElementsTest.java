...
public class ViewListElementsTest {
  ...
  public void testJavaStreamHell() throws Exception {
    ...
    List<Integer> indices =
        IntStream.range(1, maxExclusive + 1).boxed().collect(Collectors.toList());
  ...
  private List<Integer> collectIndicesOf(List<String> team, String mate) {
    ...
    List<Integer> indices =
        IntStream.range(1, team.size() + 1)
            .filter(i -> index.compareAndSet(i, i + 1) & team.get(i - 1).contains(mate))
            .boxed()
