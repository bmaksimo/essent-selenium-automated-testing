...
public class NumericUtil {
  ...
  private static final String EQUALITY_REGEX_EXT =
      "(?<=(not equal|greater than|greater than or equal|less than|less than or equal))?\\s*(\\d+)";
  private static final Map<String, BiFunction<Integer, Integer, Boolean>> equalityOperators =
      new HashMap<>();
  ...
  public static String extractFirstNumericPart(String input) {
    ...
    if (matcher.find()) {
      ...
      throw new CucumberException(
          format(
              "--Billing customer and tariff date input '%s' doesn't match the pattern '%s'",
              input, NUMERIC_AND_ANYTHING_REGEX));
    }
  }
  ...
  public static int amountAsInt(String amountInCurrency, Locale locale) {
    ...
    try {
      ...
      throw new CucumberException(
          "Exception while parsing the advance amount string " + amountInCurrency);
    }
  }
  ...
  public static boolean checkAmount(int actualAmount, String expressionExpectedAmount) {
    ...
    throw new CucumberException(
        "Expression '" + expressionExpectedAmount + "' did not contain expected equality term.");
  }
