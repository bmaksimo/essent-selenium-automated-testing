...
public class DateTimeFormatUtil {
    ...
    private static final PeriodFormatter PERIOD_FORMATTER = new PeriodFormatterBuilder().appendHours().appendSuffix(" hour(s)")
            .appendSeparator(", ").appendMinutes().appendSuffix(" minute(s)")
            .appendSeparator(", ").appendSeconds().appendSuffix(" second(s)")
            .appendSeparator(", ").appendMillis().appendSuffix(" milli(s)").toFormatter();
