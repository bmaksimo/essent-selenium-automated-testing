...
/**
 * Support of input- and output- parameters (fields annotated with @OutputParameter
 * and @InputParameter defined in subclasses of RegisteredScenario )
 */
public class ParametersUtil {
  ...
  public static void collectScenarioOutputParameters(
      RegisteredScenario registeredScenario, BiConsumer<String, Object> putToParameterProvider) {
      ...
      if (declaredField.isAnnotationPresent(OutputParameter.class)) {
        ...
        try {
          ...
          if (parameterNames.contains(parameterName)) {
            logger.info(
                " - ACTION: OUT_PARAM_OVERRIDE: Overriding output Parameter"
                    + " '"
                    + parameterName
                    + "': "
            ...
            logger.info(
                " - ACTION: OUT_PARAM_NEW: Output Parameter"
                    + " '"
                    + parameterName
                    + "': "
                    + (parameterValue == null ? "null" : parameterValue.toString()));
  ...
  public static <T extends Annotation> void assignOutValuesToInputParameters(
      Function<String, Object> outParamProvider, RegisteredScenario registeredScenario) {
    Class<?> objectClass = registeredScenario.getClass();
    List<Field> fields = Arrays.asList(objectClass.getDeclaredFields());
    fields.stream()
        .filter(
            p -> {
              return p.isAnnotationPresent(InputParameter.class);
            })
        .forEach(
            field -> {
              ...
              try {
                ...
                logger.info(
                    " - ACTION: IN_PARAM_ASSIGN_VALUE: Assigning "
                        + (outputParameter == null ? "null" : outputParameter.toString())
                        + " to "
                ...
                logger.error(
                    " - FAILURE:  Failure accessing the InputParameter '"
                        + annotation.name()
                        + "'");
                throw new CucumberException(
                    "Error accessing the InputParameter '" + annotation.name() + "'");
              }
            });
  }
}
