...
public class EssentPrettyFormatter extends PrettyFormatter implements ColorAware {
  ...
  static {
    annotationRules.put(
        OutputParameter.class, (BiConsumer<String, Object>) EssentPrettyFormatter::accept);
  }
  ...
  public void result(Result result) {
    ...
    if (Result.PASSED.equals(result.getStatus())) {
      ...
      parameterProvider
          .consumingNullValues(true)
          .put("cucumber-scenario-failure", result.getError());
    }
  ...
  private void assignInputFromOuputParameters(RegisteredScenario activeScenario) {
    ...
    ParametersUtil.assignOutValuesToInputParameters(
        (Function<String, Object>) (s) -> parameterProvider.get(s), activeScenario);
  }
  ...
  private RegisteredScenario getActiveScenario(String scenarioName) {
    ...
    RegisteredScenario activeScenario =
        provideNotNull(
            ActiveScenarioProvider.get().getActiveScenario(name),
            String.format(
