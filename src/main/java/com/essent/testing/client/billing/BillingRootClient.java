...
public class BillingRootClient {
  ...
  /**
   * Call the REST service and return the result. Exceptions are handled here, but null is returned
   * when that happens.
   *
   * <p>the RestURL must have been set before this method can be succesfully called.
   *
   ...
   */
  protected <T extends RestResponse> T call(
      String methodUrl, Object request, Class<T> responseClass) {
    if (restUrl == null) {
      throw new AssertionError(
          "BillingRootClient.call invoked, but restUrl not set, fix class "
    ...
    try {
      ...
      throw new AssertionError(
          "Failed to call JBilling, url='" + url + "', exception=" + e.getMessage());
    }
  }
  ...
  private <T extends RestResponse> T call(
      String methodUrl, Class<T> responseClass, boolean verbose) {
  ...
  private <T extends RestResponse> T call(
    ...
    try {
      ...
      ResponseEntity<T> response =
          template.exchange(
              builder.build().encode().toUri(), HttpMethod.GET, httpEntity, responseClass);
      ...
      throw new AssertionError(
          "Failed to call JBilling, url='" + url + "', excpetion=" + e.getMessage());
    }
  }

  public <T extends RestResponse> T callWithStringPayload(
      String methodUrl, String request, Class<T> responseClass) {
  ...
  public <T extends RestResponse> T callWithQueryParams(
      String methodUrl, MultiValueMap<String, String> queryParams, Class<T> responseClass) {
