...
public class TablePage extends Component {
  ...
  public boolean clickTextLink(String label) {
    WebElement we =
        seleniumDriver.findElementWhenVisible(By.xpath("//a[contains(text(),' " + label + "')]"));
    if (we != null) {
  ...
  public boolean clickFirstCellInTable(String firstCellValue) {
    WebElement we =
        seleniumDriver.findElementWhenVisible(
            By.xpath("//a[strong[contains(text(),'" + firstCellValue + "')]]"));
    if (we != null) {
      seleniumDriver.waitAndClick(we);
      return true;
    }
  ...
  public boolean clickOnRowInTable(String rowNumber) {
    ...
    WebElement we =
        seleniumDriver.findElementWhenVisible(
            By.xpath("//table//tr[" + rowNumber + "]/td[" + rowNumber + "]/a"));
    if (we != null) {
      seleniumDriver.waitAndClick(we);
      return true;
    }
  ...
  public String checkValueNextToLabel(String label) {
    return seleniumDriver
        .findElementWhenVisible(By.xpath("//tr[td[contains(text(),'" + label + "')]]/td[2]"))
        .getText();
  }
  ...
  public boolean checkInnerTablesNotEmpty() {
    ...
    List<WebElement> rows =
        seleniumDriver.findElements(By.xpath("//table[@class='innerTable']/tbody"));
