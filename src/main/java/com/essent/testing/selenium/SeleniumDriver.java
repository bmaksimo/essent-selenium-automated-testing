...
public abstract class SeleniumDriver {
  ...
  private static final String JQUERY_IS_NOT_ACTIVE =
  ...
  public void createWebDriver() {
    ...
    if (StringUtils.isNotEmpty(headless)) {
      ...
      String windowSize =
          ConfigProvider.getProperty(ConfigKey.WEBDRIVER_CHROME_HEADLESS_WINDOW_SIZE);
    ...
    chromeDriver
        .manage()
        .timeouts()
        .implicitlyWait(3, TimeUnit.MINUTES)
  ...
  public void takeScreenshot(boolean success) {
    ...
    try {
      FileUtils.copyFile(
          screenshot, new File(FilenameUtils.concat(currentAbsolutePath, screenshot.getName())));
    } catch (IOException e) {
  ...
  public void takeScreenshot(String name) {
    ...
    try {
      FileUtils.copyFile(
          screenshot,
          new File(FilenameUtils.concat(currentAbsolutePath, name + "_" + screenshot.getName())));
    } catch (IOException e) {
      logger.warn(String.format("- ACTION: failed copying screenshot to %s", currentAbsolutePath));
    }
  }
  ...
  public void awaitJqueryNotActive(long milliseconds) {
    new org.openqa.selenium.support.ui.WebDriverWait(driver, milliseconds)
        .until(
            webDriver -> {
              final JavascriptExecutor js = (JavascriptExecutor) driver;
              return (Boolean) js.executeScript(JQUERY_IS_NOT_ACTIVE);
            });
  }
}
