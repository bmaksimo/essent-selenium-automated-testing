...
public class ContractTC2B2CCreator extends QuoteCreatorB2CBase implements QuoteCreator {

	private static final Logger logger = Logger.getLogger(ContractTC2B2CCreator.class);

	public ContractTC2B2CCreator() throws FileNotFoundException, IOException{
		super();
		getQuoteProperties(ContractConstants.PATH_TO_PROPERTIES_FILE_CREATE_QUOTE_TC2, "FAKE", SwitchTypes.MOVE_IN.getSwitchTypes());
	}
	...
	public void setPreconditions(String accountName, String contractStartDate, String contractEndDate) throws Exception{
		...
		super.setPreconditions(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, accountName, contractStartDate, contractEndDate);
	...
	public String createContract() throws Exception {
		...
		login();
		setPreconditions(ContractConstants.ACCOUNT_NAME_PREFIX_TC2_B2C, upStartDate, PrepareDataForContract.getTodayDate());
		createQuote(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, ContractConstants.PATH_TO_JSON_FILES_CREATE_QUOTE_B2C_TC2, ApiPathsContract.API_CREATE_QUOTE_B2C);
		verifyQuoteStatus(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, ContractConstants.SENT_TO_CUSTOMER_EN.toUpperCase(), ContractConstants.ACCEPTED_EN.toUpperCase());
		sendToCustomer(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C);
		verifyQuoteStatus(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, ContractConstants.SENT_TO_CUSTOMER_EN.toUpperCase(), ContractConstants.ACCEPTED_EN.toUpperCase());
        signatureReceived(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, pricingDate, priceValidUntilDate, signatureReceivedDate);
		verifyQuoteStatus(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, ContractConstants.SIGNATURE_RECEIVED_EN.toUpperCase(), ContractConstants.ACCEPTED_EN.toUpperCase());
		confirmSigning(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, ContractConstants.PATH_TO_JSON_FILES_SIGN_QUOTE_MODAL_TC2_UP, ApiPathsContract.API_SIGN_QUOTE_MODAL_TC2_UP);
		signMandatePaper(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C);
		verifyContractCreated(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C, ContractConstants.SIGNED_EN.toUpperCase(), ContractConstants.ACCEPTED_EN.toUpperCase());
	...
	public String createContractAndCheckContractStatus() throws Exception {
		...
		if (numberOfAttempts < ContractConstants.MAX_NUMBER_OF_ATTEMPTS_TO_FIND_APPROPRIATE_START_CONTRACT_DATE) {
			...
			contractStatus = checkContractIsActive(ContractConstants.PATH_TO_JSON_FILES_QUOTE_TC2_B2C);
