...
public class QuoteCreatorB2BBase {
  ...
  protected void signMandatePaper(String path) throws IOException {
    ...
    billingId =
        new JsonPath(response.getBody().asString()).get("data.rows[0].rowData.billingcustomerid");
    ...
    Map<String, String> modelMandatePaper =
        new JsonPath(response.getBody().asString()).get("data.arguments.model");
  ...
  protected void createQuoteB2B(String path, String pathJsonFile, String pathApiPath)
      throws IOException {
    ...
    String payloadCreateQuoteB2B = path + pathJsonFile + ".template";
    String originalPayloadCreateQuoteB2B = path + pathJsonFile;
    ...
    String jsonBody =
        PrepareDataForContract.createRequestJsonPayload(
            payloadCreateQuoteB2B, originalPayloadCreateQuoteB2B, testMap);

    if (ConfigProvider.getProperty(ConfigKey.PROXY_ENABLE).equals("yes")) {
      RestAssured.proxy =
          ProxySpecification.host(ConfigProvider.getProperty(ConfigKey.PROXY_HOST))
  ...
  protected void getQuoteProperties(String path, String isFakeAddress, String switchType)
    ...
    pricingDate =
        ContractConstants.PATH_TO_PROPERTIES_FILE_CREATE_QUOTE_UP.equalsIgnoreCase(path)
            ? getPricingDateToday()
            : prop.getProperty("pricing_date");
    ...
    upStartDate =
        ContractConstants.PATH_TO_PROPERTIES_FILE_CREATE_QUOTE_UP.equalsIgnoreCase(path)
            ? getUPStartDate()
            : prop.getProperty("up_start_date");
    upEndDate =
        ContractConstants.PATH_TO_PROPERTIES_FILE_CREATE_QUOTE_UP.equalsIgnoreCase(path)
            ? getUPEndDate()
            : prop.getProperty("up_end_date");
  ...
  private String getUPEndDate() {
    ...
    LocalDate upEndDate =
        oneYearFromNow.minusMonths(1).withDayOfMonth(oneYearFromNow.minusMonths(1).lengthOfMonth());
    return upEndDate.format(dwpDateTimeFormatter);
  }

  protected void getQuoteProperties(String path, QuoteB2B quoteB2B)
      throws FileNotFoundException, IOException {
  ...
  private String getCurrentContractStartDateFromDWP(String path) throws IOException {
    ...
    String jsonBody =
        PrepareDataForContract.createRequestJsonPayload(
            payloadFilterByEan, originalpayloadFilterByEan, "${ean_c}", eanCode);
    ...
    Map<String, Map<String, String>> isContractExist =
