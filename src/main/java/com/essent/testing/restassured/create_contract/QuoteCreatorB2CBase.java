...
public class QuoteCreatorB2CBase {
	...
	protected void signatureReceived(String path, String pricingDate, String priceValidUntilDate, String signatureReceivedDate) throws IOException {
		...
		response = RestAssured.given().cookies(cookie).contentType("multipart/form-data")
				...
				.formParams(createFormParamsMap(rowId, accountName, recordId, yesterdayDate, aosProductsQuotesId)).when().post(ApiPathsContract.API_FILE_UPLOAD).then().statusCode(200).extract()
				.response();
	...
	protected void createQuoteB2C(String path, String pathJsonFile, String pathApiPath) throws IOException {
		...
		String payloadCreateQuoteB2C = path
				+ pathJsonFile + ".template";
		String originalPayloadCreateQuoteB2C = path
				+ pathJsonFile;
		...
		String jsonBody = PrepareDataForContract.createRequestJsonPayload(payloadCreateQuoteB2C, originalPayloadCreateQuoteB2C, testMap);
	...
	private Map<String, String> createFormParamsMap(String rowId, String accountName, String recordId, String yesterdayDate, String aosProductsQuotesId){
