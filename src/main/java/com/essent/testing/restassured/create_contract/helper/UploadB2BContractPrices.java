...
public class UploadB2BContractPrices {
  ...
  private static final String ftpDomain =
      ConfigProvider.getProperty(ConfigKey.ENVIRONMENT) + "-sftp.nova.essent.be";
  ...
  private static final String privateKeyPassword =
      ConfigProvider.getProperty(ConfigKey.SSH_PASSPHRASE); // example: can be empty string also
  private static final String locationOfPrivateKey =
      ConfigProvider.getProperty(ConfigKey.SSH_KEYPATH); // example: ..\id_rsa
  private static final String ftpUserName =
      ConfigProvider.getProperty(ConfigKey.SSH_USER); // example: dj.kovacevic

  // Method upload prices to specific location on sftp, which them will be picked by CRM and
  // imported in CRM
