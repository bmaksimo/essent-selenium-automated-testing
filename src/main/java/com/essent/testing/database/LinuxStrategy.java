...
public class LinuxStrategy implements OsStrategy {
    ...
    private void checkURLForEncodedChars(URL url) {
        ...
        if (!decodedURL.equals(url.getPath())) {
            throw new IllegalArgumentException("No paths supported with encoded chars (spaces, etc..), path is " + url.getPath());
        }
    }
    ...
    public ProcessBuilder getRestoreCmd(String host, int port, String userId, String db, String dumpLocation) {
        ...
        process.command("psql", "-w", "-h", host, "-p", "" + port, "-U", userId, "-d", db, "-f", dumpLocation);
        return process;
    }
    ...
    public ProcessBuilder getDumpCmd(String host, int port, String userId, String db, String dumpLocation) {
        ProcessBuilder process = new ProcessBuilder();
        process.command("pg_dump", "-w", "-h", host, "-p", "" + port, "-U", userId, "-d", db, "-c", "-f", dumpLocation);
        return process;
    }
