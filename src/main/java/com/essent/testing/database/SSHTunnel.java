...
class SSHTunnel {
  ...
  private static final Logger logger = Logger.getLogger(SSHTunnel.class);
  ...
  /**
   * Warning: A tunnel proved to be very slow on the REG02 environment, restoring a dump took a
   * minute against <5s without tunnel. THe code is still here, but it is very tricky.
   *
   * @param sshHostname
   * @param sshRemoteport
   * @param sshDbHostname
   ...
   */
  private void makeTunnel(String sshHostname, int sshRemoteport, String sshDbHostname)
      throws JSchException {
    if (useTunnel()) {
      // real tunnel
      logger.debug("STEP:");
      ...
      while (!tunnelCreated) {
        try {
          ...
          logger.debug(
              String.format(
                  " - RESULT: Tunnel to %s, ssh_keypath: %s, userid: %s made successfully.",
                  sshDbHostname, path, sshUser));
  ...
  private void makeTunnel(
      ConfigKey sshHostnameConfigKey,
      ConfigKey sshRemoteportConfigKey,
      ConfigKey sshDbHostnameConfigKey)
      throws JSchException {
    makeTunnel(
        ConfigProvider.getProperty(sshHostnameConfigKey),
  ...
  private void makeBillingTunnel() {
    try {
      this.makeTunnel(
          ConfigKey.SSH_BILLING_HOSTNAME,
          ConfigKey.SSH_BILLING_REMOTE_PORT,
          ConfigKey.BILLING_DB_HOST);
    } catch (JSchException e) {
  ...
  private void makeSuiteCRMTunnel() {
    try {
      makeTunnel(
          ConfigKey.SSH_SUITE_HOSTNAME, ConfigKey.SSH_SUITE_REMOTE_PORT, ConfigKey.SUITE_DB_HOST);
    } catch (JSchException e) {
   ...
   * @throws IllegalStateException if unable to find a free remotePort
   */
  private static int findFreePort() {
    ...
    throw new IllegalStateException(
        "Could not find a free TCP/IP remotePort to create a tunnel on");
  }
}
