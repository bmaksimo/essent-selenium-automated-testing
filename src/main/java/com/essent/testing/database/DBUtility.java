...
public class DBUtility {
    ...
    private static List<String> distinctEventsOnNovaAuditLog(Connection conn, String tableName) throws SQLException {
    ...
    public static void addJsonAuditLogOnBillingTable(String tableName) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBillingConnection()) {
            addJsonAuditLogOnBillingTable(conn, tableName);
        }
    }

    public static void addAsyncAuditLogOnBillingTable(String tableName) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBillingConnection()) {
    ...
    public static void addTraceOnBillingTable(String tableName) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBillingConnection()) {
            String sql = " CREATE TRIGGER trace_trigger BEFORE INSERT OR UPDATE OR DELETE ON  " + tableName
    ...
    private static void addJsonAuditLogOnBillingTable(Connection conn, String tableName) throws SQLException {
        String sql = " CREATE TRIGGER audit_trigger BEFORE INSERT OR UPDATE OR DELETE ON  " + tableName
    ...
    private static void addAsyncAuditLogOnBillingTable(Connection conn, String tableName) throws SQLException {
        String sql = " CREATE TRIGGER audit_trigger BEFORE INSERT OR UPDATE OR DELETE ON  " + tableName
    ...
    public static void checkBillingRow(String tableName, String keyName, long id, Map<String, String> fields)
    ...
    public static Date getMaxDateBilledGridfeeInvoices(String deliveryPoint) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getEdielConnection()) {
            return retrieveMexDateBilleddGridfeeInvoices(conn, deliveryPoint);
        }
    }

    public static Map<String, BigDecimal> getSumConsumptionPerTimeframe(String deliveryPoint, Date fromDate,
            Date untilDate, Boolean rectified) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getEdielConnection()) {
            return retrieveSumConsumptionPerTimeframe(conn, deliveryPoint, fromDate, untilDate, rectified);
        }
    }

    public static Date getNextBillableDateForRecurringOrderOfSettlement(String deliverypoint, String contractlineid,
            Date startdate, String nextBillableDate) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBillingConnection()) {
            return retrieveNextBillableDateForRecurringOrderOfSettlement(conn, deliverypoint, contractlineid, startdate,
                    nextBillableDate);
        }
    }

    private static Date retrieveNextBillableDateForRecurringOrderOfSettlement(Connection conn, String deliverypoint,
            String contractlineid, Date startdate, String nextBillableDate) throws SQLException {
        String sql = "select b.next_billable_day from settlement a, purchase_order b  where "
    ...
    private static Map<String, BigDecimal> retrieveSumConsumptionPerTimeframe(Connection conn, String deliveryPoint,
            Date fromDate, Date untilDate, Boolean rectified) throws SQLException {
        String sql = "select  b.timeframe , sum (value)  from consumption a, calculated_register b "
    ...
    private static List<String> retrieveUnbilledGridfeeInvoices(Connection conn, String deliveryPoint, Date untilDate)
            throws SQLException {
        String sql = "select invoice_number from gridfee_invoice where status = 0 and deliverypointid = ? and valid_from < ?";
    ...
    private static Date retrieveMexDateBilleddGridfeeInvoices(Connection conn, String deliveryPoint)
        ...
        String sql = "select max(valid_to) from gridfee_invoice  where status = 1 and deliverypointid = ?";
    ...
    private static void checkDBRow(Connection conn, String tableName, String keyName, long id,
     ...
     * @param expected
     * @param ix
     *            index of key in rsmd (and rs)
     * @throws SQLException
     */
    private static void checkField(ResultSet rs, ResultSetMetaData rsmd, String key, String expected, int ix)
            throws SQLException {
        ...
        default:
            if (expected.isEmpty()) {
                ...
                Assert.assertTrue("Field " + key + " is not null or empty but " + value,
    ...
    private static String buildSelectSql(String tableName, Map<String, String> fields, String whereCondition) {
    ...
    public static int countBillingRecords(String tableName, String key, long id) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBillingConnection()) {
            return countRecords(conn, tableName, key, id);
        }
    }

    private static int countRecords(Connection conn, String tableName, String key, long id) throws SQLException {
    ...
    public static void awaitLatestValueInTableNotToBe(String table, String column, String expectedValue, int timeout)
            throws Throwable {
    ...
    public static List<Map<String, String>> selectRecordInTableForeignKey(String tableName, String foreignKeyColumnName,
            long foreignKeyId) throws SQLException, JSchException {
        List<Map<String, String>> records = new ArrayList<>();
        try (Connection conn = new DBConnector().getBillingConnection()) {
            String sql = "SELECT * FROM " + tableName + " WHERE " + foreignKeyColumnName + " = ? order by id";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public static void setDunningAccountLastProcessedDate(String externalId, LocalDate firstDate)
        ...
        try (Connection conn = new DBConnector().getBillingConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                if (count != 1) {
                    throw new SQLException("Update dunning account.last_processed_date failed, hitcount=" + count);
                }
            }
        }
    }
    ...
    public static void uploadAndApplyDunningBRE(String name) throws SQLException, JSchException, IOException {
        ...
        try (Connection conn = new DBConnector().getBillingConnection()) {
            ...
            sql = "INSERT INTO dunning_bre (name, description, valid_from, decision_tables_json) VALUES (?, 'yadda yadda', '2000-01-01', ?)";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public static void setDunningAccountOutstandingPayments(String externalId, BigDecimal outstandingPayments)
            throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBillingConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                int count = stmt.executeUpdate();
                if (count != 1) {
                    throw new SQLException("Update dunning account.outstanding_payments failed, hitcount=" + count);
                }
            }
        }
    }
    ...
    public static void advanceDunningAccountLastProcessedDateWithDays(String externalId, int nr_days)
        ...
        try (Connection conn = new DBConnector().getBillingConnection()) {
            ...
            // at or near "$1", pos 81
            String sql = "UPDATE dunning_account " + //
                    "SET last_processing_date=last_processing_date + interval '" + nr_days + "' day " + //
                    "WHERE external_id = ?";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                stmt.setString(1, externalId);
                int count = stmt.executeUpdate();
                if (count != 1) {
                    throw new SQLException("Advancing dunning account.last_processed_date failed, hitcount=" + count);
                }
            }
        }
    ...
    /**
     * Return true or false depending on the value. If it starts with t or T we
    ...
    public static void setDunningAccountActive(String externalId, String active) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBillingConnection()) {
            String sql = "UPDATE dunning_account " + //
                    "SET flag_active=? " + //
                    "WHERE external_id = ?";
    ...
    public static void scheduleDunningInboundCommForInvoice(String invoiceNr, String eventName)
        ...
        long invoiceId = DBUtility.getInvoiceId(invoiceNr);
        try (Connection conn = new DBConnector().getBillingConnection()) {
            String sql = "INSERT INTO dunning_event " + //
    ...
    public static void validateDunningEventsBasedOnInvoice(long invoice_id, String type, LocalDate date, String message,
            int nr_records) throws SQLException, JSchException {

        check_nr_dunning_records("invoice_id", invoice_id, type, date, message, nr_records);
    }

    public static void validateDunningEventsBasedOnBundle(long bundle_id, String type, LocalDate date, String message,
            int nr_records) throws SQLException, JSchException {

        check_nr_dunning_records("bundle_id", bundle_id, type, date, message, nr_records);
    }

    private static void check_nr_dunning_records(String id_field, long id_value, String type, LocalDate date,
            String message, int nr_records) throws SQLException, JSchException {
        Date sqlDate = (date == null) ? null : new Date(date.toDate().getTime());
        try (Connection conn = new DBConnector().getBillingConnection()) {
            String sql = "SELECT count(*) " + //
                    "  FROM dunning_event " + //
                    "WHERE " + id_field + "=? AND event_type=? AND message=? " + //
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        String msg = String.format(
                                "Incorrect number of records for [%s:%d, type:%s, date:%s, message:%s]", id_field,
                                id_value, type, String.valueOf(date), message);
    ...
    public static void setInvoiceDates(long invoiceId, LocalDate creationDate, LocalDate dueDate)
        ...
        try (Connection conn = new DBConnector().getBillingConnection()) {
            String sql = "UPDATE invoice SET create_datetime=?, create_timestamp=?, due_date=? WHERE id=?";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                int count = stmt.executeUpdate();
                if (count != 1) {
                    throw new SQLException("Update dunning_invoice " + invoiceId + " failed, hitcount=" + count);
                }
            }
        }
    }
     ...
     * @throws SQLException
     */
    public static String getPublicNumberForInvoiceId(long invoiceId) throws SQLException, JSchException {
     ...
     *
     * @param tableName
     *            table to check (from billing database)
     * @param count
     *            nr of records expected
     * @param fields
     *            which fields are present in the where clause
     * @throws JSchException
     * @throws SQLException
     * @throws AssertionError
     *             with the SQL as message if count is not correct.
     */
    ...
    private static void checkTableRows(Connection conn, int count, String tableName, Map<String, String> fields)
            throws SQLException, JSchException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql.toString())) {
            ...
            for (int ix = 0; ix < values.size(); ix++) {
                ...
                default:
                    throw new SQLException("Unsupported sql type " + fieldType.get(ix) + " with value " + val);
                }
            }
    ...
    private static ResultSetMetaData getMetaDataForTable(Connection conn, String tableName)
            throws SQLException, JSchException {
        String sql = "SELECT * FROM " + tableName + " WHERE 1 = 0"; // fast query to obtain the metadata.
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            try (ResultSet rs = stmt.executeQuery()) {
    ...
    public static List<String> getEansForBundle(long bundle_id) throws Exception {
        ...
        String sql = "SELECT mfv.string_value FROM dunning_bundle db " + //
        ...
        try (Connection conn = new DBConnector().getBillingConnection()) {
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    ...
                    if (results.isEmpty()) {
                        throw new SQLException("Obtaining delivery point id's for bundle " + bundle_id + " failed");
                    }
                    return results;
                }
            }
    ...
    public static void setEdielServiceValidTo(String deliveryPointId, LocalDate effectiveDate)
        ...
        String sql = "UPDATE service SET valid_to=? " + //
                "WHERE id IN (" + //
    ...
    private static List<Map<String, Object>> getBilledBalancePeriods(Connection conn, long settlementId)
            throws SQLException {
    ...
    private static List<Map<String, Object>> getUnbilledBalancePeriods(Connection conn, long settlementId)
            throws SQLException {
    ...
    private static String getCustomerMetaFieldSql(String metaFieldName, String alias) {
        ...
        return String.format("" + "left outer join ( "
                ...
                + ") %s on %s.customer_id = cust.id ", metaFieldName, alias, alias);
        // @formatter:off
    }

    public static boolean getCustomerBundlingActive(String billingId) throws SQLException, JSchException {
        boolean result = false; // default no
        try (Connection conn = new DBConnector().getBillingConnection()) {
            // @formatter:off
            String sql = "select boolean_value from meta_field_value mfv "
    ...
    public static BigDecimal getBalanceReversedAmount(long settlementId) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBalanceConnection()) {
            return getBalanceReversedAmount(conn, settlementId);
        }
    }

    public static BigDecimal getBalanceTotalAmount(long settlementId) throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getBalanceConnection()) {
            return getBalanceTotalAmount(conn, settlementId);
        }
    }

    private static BigDecimal getBalanceTotalAmount(Connection conn, long settlementId) throws SQLException {
    ...
    private static BigDecimal getBalanceReversedAmount(Connection conn, long settlementId) throws SQLException {
    ...
    public static void setPreferenceType(String preference, String value) throws Exception {
        String sql = "" +
        // @formatter:off
                "update preference_type " + "set def_value = ? "
                + "where id = (select foreign_id from international_description ides " + "where table_id = 50 "
                + "and psudo_column = 'description' " + "and language_id = 1 " + "and content = ?) ";
        // @formatter:on
        try (Connection conn = new DBConnector().getBillingConnection()) {
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public static void setPreference(String preference, String value) throws Exception {
        // @formatter:off
        String sql = "" + "update preference " + "set value = ? "
                + "where type_id = (select foreign_id from international_description ides " + "	where table_id = 50 "
                + "	and psudo_column = 'description' " + "	and language_id = 1 " + "	and content = ?) "
                + "and table_id = 5 " + "and foreign_id = 60 ";
        // @formatter:on
        try (Connection conn = new DBConnector().getBillingConnection()) {
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                stmt.setString(1, value);
    ...
    public static Long getSettlementField(String fieldName, String ean_or_contractline, Date startDate, Integer status)
            throws Exception {
        // @formatter:off
        // If there are more settlements with the same key (Yes, it happens in old
        // databases), take the newest one
        String sql = "select " + fieldName + " from settlement "
                + "where (contract_line_id=? or consumption_key=?) and start_date=? and status=? " + "order by id desc";
        // @formatter:on
        try (Connection conn = new DBConnector().getBillingConnection()) {
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public static List<Long> getInvoiceOrdersField(String fieldName, String publicNumber) throws Exception {
        // @formatter:off
        String sql = "select op." + fieldName
    ...
    public static Long getCustomerField(String fieldName, String userName) throws Exception {
        // @formatter:off
        String sql = "select c." + fieldName + " from customer c " + "inner join base_user bu on c.user_id=bu.id "
                + "where user_name=?";
        // @formatter:on
        try (Connection conn = new DBConnector().getBillingConnection()) {
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public static Long getServiceField(String fieldName, String deliveryPoint, Date startDate) throws Exception {
        // @formatter:off
    ...
    public static Long getOrderProcessField(String fieldName, String publicNumber) throws Exception {
        // @formatter:off
        String sql = "select " + fieldName + " from order_process op " + "inner join invoice i on i.id=op.invoice_id "
                + "where public_number = ?";
        // @formatter:on
        try (Connection conn = new DBConnector().getBillingConnection()) {
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public static void UpdateMeteringContextForConsumption(String deliverypoint, String meteringContext)
            throws SQLException, JSchException {
        try (Connection conn = new DBConnector().getEdielConnection()) {
    ...
    private static void UpdateMeteringContextForConsumption(Connection conn, String deliverypoint,
            String meteringContext) throws SQLException {
        // @formatter:off
    ...
    private static void resetEnergyCommForDeliveryPoint(Connection conn, String deliverypoint) throws SQLException {

        List<String> sqls = new ArrayList<>();

        // @formatter:off
        final String adapter1 = " delete from ediel.edieltransaction where envid in ( "
                + "select cast(substring(ref, position ('=' in ref)+1,position (',' in ref) -position ('=' in ref) - 1) as int) "
                + "from (  select distinct externalmessageid as ref " + "from kernel.message m  "
                + "left join kernel.processmessages pm on m.messageid = pm.messageid "
                + "left join kernel.process p on pm.processid = p.processid " + "where m.gsrn  = ? "
                + "and msgdir = 'IN' and counterpart = 'EdielAdapter' " + ") as s " + "); ";

        sqls.add(adapter1);

        final String adapter2 = "  delete from ediel.msgctrl where envid in ( "
                + "select cast(substring(ref, position ('=' in ref)+1,position (',' in ref) -position ('=' in ref) - 1) as int) "
                + "from (  select distinct externalmessageid as ref " + "from kernel.message m "
                + "left join kernel.processmessages pm on m.messageid = pm.messageid "
                + "left join kernel.process p on pm.processid = p.processid " + "where m.gsrn  = ? "
        ...
        final String adapter3 = "delete from ediel.msgctrl where refenvid in ( "
                + "select cast(substring(ref, position ('=' in ref)+1,position (',' in ref) -position ('=' in ref) - 1) as int) "
                + "from (  select distinct externalmessageid as ref " + "from kernel.message m "
                + "left join kernel.processmessages pm on m.messageid = pm.messageid "
                + "left join kernel.process p on pm.processid = p.processid " + "where m.gsrn  = ? "
        ...
        final String adapter4 = " delete from ediel.msgenv where envid in ( "
                + "select cast(substring(ref, position ('=' in ref)+1,position (',' in ref) -position ('=' in ref) - 1) as int) "
                + "from (  select distinct externalmessageid as ref " + "from kernel.message m "
                + "left join kernel.processmessages pm on m.messageid = pm.messageid "
                + "left join kernel.process p on pm.processid = p.processid " + "where m.gsrn  = ? "
        ...
        final String adapter5 = "delete from ediel.edieltransaction where kernelmessageid in "
                + " ( select distinct m.messageid " + " from kernel.message m "
                + " left join kernel.processmessages pm on m.messageid = pm.messageid "
                + " left join kernel.process p on pm.processid = p.processid " + " where m.gsrn  = ? "
        ...
        final String kernel1 = " delete from kernel.deliveryperiodbrpjournal where deliveryperiodid in ( "
        ...
        final String kernel2 = "delete from kernel.transactioncrossref where processid in ( "
        ...
        final String kernel3 = " delete from kernel.processmessages where processid in ( "
                + " select processid from kernel.process where deliveryperiodid in ( "
                + " select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ( "
                + "     select deliverypointid from kernel.deliverypoint acp where gsrn = ?))); ";
        ...
        final String kernel4 = "delete from kernel.unblockingmsg where expectedmessageid in ( "
        ...
        final String kernel5 = "delete from kernel.expectedmessages where processid in ( "
                + " select processid from kernel.process where deliveryperiodid in ( "
                + "  select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ( "
                + "       select deliverypointid from kernel.deliverypoint acp where gsrn = ?))); ";
        sqls.add(kernel5);

        final String kernel6 = "delete from kernel.process where deliveryperiodid in ( "
                + "  select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ( "
                + "  select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel6);

        final String kernel7 = "delete from kernel.deliveredcustomer where deliveryperiodid in ( "
                + " select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ( "
                + "    select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel7);

        final String kernel8 = "delete from kernel.consumptiondetail where consumptionid in ( "
                + " select consumptionid from kernel.consumption where deliveryperiodid in ( "
                + "  select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ( "
                + "   select deliverypointid from kernel.deliverypoint acp where gsrn = ?))); ";
        sqls.add(kernel8);

        final String kernel9 = "delete from kernel.consumption where deliveryperiodid in ( "
                + " select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ("
                + "  select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel9);

        final String kernel10 = "delete from kernel.consumptiondetail where consumptionid in ("
                + "  select consumptionid from kernel.consumption where deliverypointid in ( "
                + "  select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel10);

        final String kernel11 = "delete from kernel.consumption where deliverypointid in ( "
                + " select deliverypointid from kernel.deliverypoint acp where gsrn = ?);";
        sqls.add(kernel11);

        final String kernel12 = "delete from kernel.indexdetail where indexid in ( "
                + "select indexid from kernel.index where deliveryperiodid in ( "
                + " select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ( "
                + "      select deliverypointid from kernel.deliverypoint acp where gsrn = ?))); ";
        sqls.add(kernel12);

        final String kernel13 = "delete from kernel.index where deliveryperiodid in ( "
                + "select deliveryperiodid from kernel.deliveryperiod where deliverypointid in ( "
                + " select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel13);

        final String kernel14 = "delete from kernel.deliveryperiod where deliverypointid in ( "
                + "select deliverypointid from kernel.deliverypoint acp where gsrn = ?);";
        sqls.add(kernel14);

        final String kernel15 = "delete from kernel.physicalregister where meterid in ( "
        ...
        final String kernel16 = "delete from kernel.meter where deliverypointid in ( "
                + " select deliverypointid from kernel.physicalconfig where deliverypointid in ( "
                + "       select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel16);

        final String kernel17 = "delete from kernel.physicalconfig where deliverypointid in ( "
                + " select deliverypointid from kernel.deliverypoint acp where gsrn = ?); ";
        sqls.add(kernel17);

        final String kernel18 = "delete from kernel.logicalregister where deliverypointid in ( "
                + "select deliverypointid from kernel.logicalconfig where deliverypointid in ( "
                + "      select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel18);

        final String kernel19 = "delete from kernel.logicalconfig where deliverypointid in ( "
                + "select deliverypointid from kernel.deliverypoint acp where gsrn = ?); ";
        sqls.add(kernel19);

        final String kernel20 = "delete from kernel.generalconfig where deliverypointid in ( "
                + "select deliverypointid from kernel.deliverypoint acp where gsrn = ?); ";
        sqls.add(kernel20);

        final String kernel21 = "delete from kernel.estimatedetail where estimateid in ( "
                + "select estimateid from kernel.estimate where deliverypointid in ( "
                + "       select deliverypointid from kernel.deliverypoint acp where gsrn = ?)); ";
        sqls.add(kernel21);

        final String kernel22 = "delete from kernel.estimate where deliverypointid in ( "
                + " select deliverypointid from kernel.deliverypoint acp where gsrn = ?); ";
        sqls.add(kernel22);
        ...
        final String kernel24 = "delete from kernel.processmessages where messageid in (select messageid from kernel.message where  gsrn = ?); ";
        sqls.add(kernel24);

        final String kernel25 = "delete from kernel.expectedmessages where messageid in (select messageid from kernel.message where  gsrn = ?); ";
        sqls.add(kernel25);

        final String kernel26 = "delete from kernel.messagemetadata where messageid in (select messageid from kernel.message m where gsrn = ?);  ";
        sqls.add(kernel26);

        final String kernel27 = "update kernel.message set relatedmessageid = null where gsrn = ?; ";
        sqls.add(kernel27);

        final String kernel28 = "delete from kernel.messagefeedback where messageid in (select messageid from kernel.message where gsrn  = ?);";
        sqls.add(kernel28);
    ...
    private static void ExecuteEnergycomCleanStatement(Connection conn, String sql, String deliverypoint)
            throws SQLException {
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, deliverypoint);
            stmt.executeUpdate();
    ...
    public static Integer asInteger(Object object) {
        ...
        throw new IllegalStateException(
                String.format("Class %s is not convertable to Integer by DBUtility.asLong(Object)",
                        (object == null) ? "null" : object.getClass().getName()));
    }

    /**
     * Will return the object given as a Long instance, doing conversions as needed.
     * This works better than ResultSet.getLong() as it returns a long, which means
     * it cannot handle NULL values well.
     *
     * @param object
     * @return
     */
    public static Long asLong(Object object) {
        ...
        throw new IllegalStateException(String.format("Class %s is not convertable to Long by DBUtility.asLong(Object)",
                (object == null) ? "null" : object.getClass().getName()));
    }
