...
public class DefaultListView extends Component implements ListView {
  ...
  private static final String TABLE_CELL_SELECTOR_TEMPLATE =
      "//table[@class='oe_list_content'][1]//tbody//tr[${rowIndex}]//td[@data-field='${key}'][1]";
  ...
  public void checkCellAt(String columnName, String rowIndex, String value) {
    ...
    if (!value.equals(cellText)) {
      ...
      throw new ExtendedCucumberException(
          String.format(
              "The value %s at column %s row %s does not match the expected one (%s)",
              cellText, columnName, rowIndex, value));
    }
  }
  ...
  public void clickCellAt(String columnName, String rowIndex) {
    ...
    if (CollectionUtils.isEmpty(rows)) {
      ...
      throw new ExtendedCucumberException(
          String.format("Cell at column %s, row %s was not found", columnName, rowIndex));
    } else {
  ...
  public void clickValueAt(String columnName, String value) {
    ...
    throw new ExtendedCucumberException(
        String.format("Cell at column %s having value %s was not found", columnName, value));
  }

  private boolean currentRowEquals(WebElement currentRow, String value) {
    return currentRow != null
        && currentRow.isDisplayed()
        && value.equalsIgnoreCase(currentRow.getText());
  }
  ...
  private List<WebElement> extractTable() {
    List<WebElement> rows =
        seleniumDriver.findElements(
            By.xpath("//table[@class='oe_list_content'][1]//tbody//tr"),
            Duration.ofSeconds(30),
            Duration.ofSeconds(5));
    if (rows.isEmpty()) {
