...
public abstract class RegisteredScenario {
  ...
  @Autowired protected ParameterProvider parameterProvider;
  private static final Logger logger = Logger.getLogger(RegisteredScenario.class);
  ...
  /**
   * The method will mapthe instance of active scenario to the simple scenario name. Then,
   * EssentPretyFormatter plugin methods, such as match(), gain access to active scenario
   *
   * @param scenario
   */
  ...
  protected RegisteredScenario getScenarioInstance(Class scenarioClass) {
    RegisteredScenario activeScenario =
        ActiveScenarioProvider.get().getActiveScenario(scenarioClass.getSimpleName());
    if (activeScenario == null) {
      throw new CucumberException(
          String.format(
              "Scenario %s has not been registered. Please check @Before annotation and the list of Gherkin tags (@DWP, @REGRESSION, @E2E,...).",
