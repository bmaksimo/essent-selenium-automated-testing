...
public class PersonalDetailsAddressPage extends QuoteCreationGuidedStep {
  ...
  public boolean fillInCustomerAddress() {
    ...
    if (street.equals("Random")) {
      street = AddressUtil.getRandomStreet();
    }
    ...
    initializeAddress
        .step(
            createStep(TYPING).element(DELIVERY_ADDR_STREET.name()).value(street),
            INPUT.getSleepInMillis())
        .step(
            createStep(ACCESS)
                .element(DELIVERY_ADDR_STREET_SUGGESTION.name())
                .requireDisplayed(false)
                .callback(new HideAddressSuggestion()))
        .step(
            createStep(TYPING)
                .timeoutInSeconds(3)
