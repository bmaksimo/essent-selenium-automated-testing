...
public class BaseObjectPage extends Component {
  ...
  private static final String PLUS_MENU_XPATH =
      "//list-row-action[normalize-space(@label)='${" + REPLACEMENT_KEY + "}']//a";
  private static final String PLUS_MARK_DONE_XPATH =
      "//list-row-action/a/span[@class='icon-checkmark']";

  private static final String DATE_SELECTOR_XPATH =
      "//validation-wrapper[@label='${" + REPLACEMENT_KEY + "}']//input";
  ...
  private static final String LABEL_CLICK_XPATH =
  ...
  private static final String CARD_TEXT_XPATH =
      "//h2[normalize-space(text())='${"
          + REPLACEMENT_KEY
          + "}']/parent::div/parent::div/div[@class='form__group']//label[normalize-space(text())='${"
          + REPLACEMENT_KEY1
          + "}']/parent::div//strong";
  private static final String CHECKBOX_XPATH =
      "//label[text()[contains(.,'${" + REPLACEMENT_KEY + "}')]]//input[@type='checkbox']";
  ...
  public void plusSubAction(String actionValue) {
    ...
    try {
      seleniumDriver.waitAndClick(
          seleniumDriver.findElementWhenClickable(By.xpath(xpathSubaction)));
  ...
  public void clickOnMarkAsDonePlusMenuSubAction() {
    seleniumDriver.waitForRequestsToFinish();
    seleniumDriver.waitAndClick(
        seleniumDriver.findElementWhenVisible(By.xpath(PLUS_MARK_DONE_XPATH)));
  }
  ...
  public void dateIsNow(String labelValue) {
    ...
    seleniumDriver.waitAndSendKeys(
        findElementWhenVisible(By.xpath(xpathDate)), SIMPLE_DATEF_ORMAT.format(new Date()));
  ...
  public void confirmQuote() {
    ...
    seleniumDriver.waitAndClick(
        seleniumDriver.findElementWhenVisible(By.cssSelector(NEXT_BUTTON.getQuery())));
    seleniumDriver.waitForRequestsToFinish();
    Sleeper.sleepTightInSeconds(5);
  }
  ...
  public void toggleCheckbox(String label, SwitchState state) {
    ...
    try {
      ...
      if (!checked.equalsIgnoreCase(checkBoxElement.getAttribute("checked")))
