...
public class ToggleImpl extends Component {
    private static String TOGGLEINPUT_SELECTOR =  "//validation-wrapper[@toggleInputName='${toggleInputName}']//toggle-form-element/toggleInputName";
    ...
    private boolean isOnWithDot(String checkBoxName)  {
        WebElement cb = getToggleInputWithDot(checkBoxName);
        seleniumDriver.waitForRequestsToFinish();
        String classValue = cb.findElement(By.cssSelector("input")).getAttribute("class");
        return classValue.contains("not-empty");
    }

    public void switchOnWithDot(String checkBoxName)  {
         seleniumDriver.waitForRequestsToFinish();
        if (!isOnWithDot(checkBoxName)) {
            seleniumDriver.waitAndClick(getToggleInputWithDot(checkBoxName));
        }
    }
}
