...
public class NewLeadPage extends BaseObjectPage implements Form {
  ...
  private void setGender(String gender) {
    ...
    seleniumDriver.waitAndClick(
        findElementWhenVisible(
            By.xpath("//select[@id='gender-c-field']/option[@label='" + gender + "']")));
    seleniumDriver.waitForRequestsToFinish();
  }
  ...
  private void setCompanyName(String companyNumber) {
    seleniumDriver.waitAndSendKeys(
        findElementWhenVisible(By.id("company-name-c-field")), companyNumber);
    seleniumDriver.waitAndClick(
        findElementWhenVisible(By.xpath(".//*[@id='company_name_c']/div/autocomplete/ul/li[4]")));
    seleniumDriver.waitForRequestsToFinish();
  }
  ...
  private void setContactPerson(String contactPersonName, String contactPersonLastName) {
    seleniumDriver.waitForRequestsToFinish();
    seleniumDriver.waitAndSendKeys(
        findElementWhenVisible(By.id("first-name-field")), contactPersonName);
    seleniumDriver.waitForRequestsToFinish();
    seleniumDriver.waitAndSendKeys(
        findElementWhenVisible(By.id("last-name-field")), contactPersonLastName);
  }

  private WebElement getTelephoneElement() {
    return seleniumDriver.findElementWhenVisible(
        By.id(
            "leads-contact-details-contact-details-phone-type-work-phone-contact-details-type-phone-contact-details-value-field"));
  }
  ...
  private void setMobile(String mobile) {
    ...
    seleniumDriver.waitAndSendKeys(
        findElementWhenVisible(
            By.id(
                "leads-contact-details-contact-details-phone-type-mobile-phone-contact-details-type-phone-contact-details-value-field")),
        mobile);
  }

  private WebElement getEmailElement() {
    return seleniumDriver.findElementWhenVisible(
        By.id("leads-contact-details-contact-details-type-email-contact-details-value-field"));
  }
