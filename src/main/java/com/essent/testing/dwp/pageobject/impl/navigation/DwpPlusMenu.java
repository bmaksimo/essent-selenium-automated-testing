...
public class DwpPlusMenu extends Component {
  ...
  private static String ACCORDION_BUTTON_SELECTOR_TEMPLATE_CHILD =
      "//menu-link[normalize-space(@label)='${" + REPLACEMENT_KEY1 + "}']//a";
  private static String ACCORDION_BUTTON_SELECTOR_TEMPLATE =
      "//labeled-accordion-wrapper[normalize-space(@label)='${" + REPLACEMENT_KEY2 + "}']";
  private static String LABELED_ACCORDION_WRAPPER_SELECTOR =
      "labeled-accordion-wrapper[label='${text}'] > a";
  private static int LEAF_SIZED = 1;
  private static int PARENT_MENU = 2;

  private static final String XPATH_CONTAINS_TEXT_TEMPLATE = "//span[contains(text(),'${text}')]";
  ...
  private WebElement findAction(String actionLabel, String parentMenu) {
    ...
    if (StringUtils.isNotBlank(parentMenu)) valuesMap.put(REPLACEMENT_KEY2, parentMenu);

    By accordionButtonSelector =
        valuesMap.size() > 1
            ? By.xpath(
                createQuery(
                    ACCORDION_BUTTON_SELECTOR_TEMPLATE
                        + "/child::div"
                        + ACCORDION_BUTTON_SELECTOR_TEMPLATE_CHILD,
                    valuesMap))
  ...
  private WebElement findMenu(WebElement item, List<String> menu) {
    ...
    By labeledAccordionWrapperBy =
        By.cssSelector(createQuery(LABELED_ACCORDION_WRAPPER_SELECTOR, "text", menuItem));
    FluentWait<WebDriver> waiter =
        new FluentWait<>(seleniumDriver.getDriver()).withTimeout(Duration.ofSeconds(5));
    WebElement result =
        waiter.until(ExpectedConditions.presenceOfElementLocated(labeledAccordionWrapperBy));
    result.click();
