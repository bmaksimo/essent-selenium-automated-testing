...
public class TableFilterBase {
  ...
  public TableFilterBase(
      String contextParameters,
      SeleniumDriver seleniumDriver,
      String tablePathBase,
      String tableRowsBase) {
  ...
  public String getTextValueFromColumn(String column) {
    ...
    return StringUtils.isNotBlank(this.getRow().get(columnIndex).getText())
        ? this.getRow().get(columnIndex).getText()
        : StringUtils.EMPTY;
  }

  private void logErrors(List<Filter> filters) {
    String currentContextParameters =
        StringUtils.isBlank(this.contextParameters) ? "" : this.contextParameters;
    logger.error(
        currentContextParameters
            + " - Table \""
            + this.tableName
            + "\" did not contain row(s) with provided filter: "
            + getProvidedFilters(filters));
    logger.error(
        currentContextParameters
            + " - Table \""
            + this.tableName
    ...
    try {
      for (Filter filter : filters) {
        ...
        if (cells
            ...
            .getText()
            .toUpperCase()
            .contains(filter.getColumnValue().toUpperCase())) {
          return true;
        } else return false;
      }
      return true;
  ...
  protected List<String> getHeaders() {
    return seleniumDriver.findElementsWithDefaultWaiting(By.xpath(this.tablePath + "//thead/tr/th"))
        .stream()
        .map(WebElement::getText)
