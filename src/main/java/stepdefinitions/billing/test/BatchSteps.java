...
public class BatchSteps extends RegisteredScenario {
  ...
  public void execute_mediation_job_for_settlement_date(
      String jobName, @Transform(DateMapper.class) Date settlementDate)
  ...
  public void execute_mediation_job_no_wait_for_settlement_date(
      String jobName, @Transform(DateMapper.class) Date settlementDate)
      throws InterruptedException {
  ...
  public void try_second_job_no_wait_for_settlement_date(
      String jobName, @Transform(DateMapper.class) Date settlementDate)
      throws InterruptedException {
  ...
  @Then(
      "^Execute mediation job \"([^\"]*)\" for billingid \"([^\"]*)\" and settlement date \"([^\"]*)\"$")
  public void execute_mediation_job_for_billingid_and_settlement_date(
      String jobName, String billingid, @Transform(DateMapper.class) Date settlementDate)
      throws Throwable {
    run_and_wait_for_mediation_job(jobName, billingid, null, settlementDate);
  }

  @Given(
      "^Execute mediation job \"([^\"]*)\" for deliverpoint \"([^\"]*)\" and settlement date \"([^\"]*)\"$")
  public void execute_mediation_job_for_deliverpoint_and_settlement_date(
      String jobName, String deliverypointId, @Transform(DateMapper.class) Date settlementDate)
      throws Throwable {
    run_and_wait_for_mediation_job(jobName, null, deliverypointId, settlementDate);
  }

  @And("^[E|e]xecute billing run \"([^\"]*)\" for process date \"([^\"]*)\"$")
  public void start_bill_run_for_process_date(
      String jobName, @Transform(DateMapper.class) Date processDate) throws Throwable {
    startBillRun(jobName, null, processDate, null, null, true, true);
  }

  @And("^[E|e]xecute billing run \"([^\"]*)\" for process date \"([^\"]*)\" no invoice created$")
  public void start_bill_run_for_process_date_no_invoice_created(
      String jobName, @Transform(DateMapper.class) Date processDate) throws Throwable {
    startBillRun(jobName, null, processDate, null, null, true, false);
  }

  @And(
      "^Execute billing run \"([^\"]*)\" for billingUser \"([^\"]*)\" and process date \"([^\"]*)\"$")
  public void start_bill_run_for_billingUser_and_process_date(
      String jobName, String billingCustomerId, @Transform(DateMapper.class) Date processDate) {
    startBillRun(jobName, billingCustomerId, processDate, null, null, true, true);
  }

  @Then(
      "^Execute billing run \"([^\"]*)\" for process date \"([^\"]*)\" and selectionDay \"([^\"]*)\"$")
  public void execute_billing_run_for_process_date_and_selectionDay(
      String jobName, @Transform(DateMapper.class) Date processDate, String selectionDay)
      throws Throwable {
  ...
  @And(
      "^Execute billing run \"([^\"]*)\" for process date \"([^\"]*)\" and invoice date \"([^\"]*)\"$")
  public void execute_billing_run_for_process_date_and_invoice_date(
      String jobName, @Transform(DateMapper.class) Date processDate, String invoiceDate)
      throws Throwable {
    // Write code here that turns the phrase above into concrete actions
  ...
  public void execute_billing_run_no_wait_for_process_date(
      String jobName, @Transform(DateMapper.class) Date processDate) throws Throwable {
    // Write code here that turns the phrase above into concrete actions
    startBillRun(
        jobName,
  ...
  public void try_billing_run_no_wait_for_process_date(
      String jobName, @Transform(DateMapper.class) Date processDate) throws Throwable {
    // Write code here that turns the phrase above into concrete actions
    startBillRun(
        jobName,
  ...
  @And(
      "^Execute billing run \"([^\"]*)\" for billingUser \"([^\"]*)\" and process date \"([^\"]*)\" and invoice date \"([^\"]*)\"$")
  public void execute_billing_run_for_billingUser_process_date_and_invoice_date(
      String jobName,
  ...
  public void run_and_wait_for_mediation_job(
      String jobName, String billingCustomerId, String deliverypointId, Date settlementDate)
      throws InterruptedException {
  ...
  public void run_and_do_not_wait_for_mediation_job(
      String jobName,
      String billingCustomerId,
    ...
    if (expectSuccess) {
      ...
      Assert.assertFalse(
          "job " + jobName + " started, but should have failed", response.getResult());
    }
  }
