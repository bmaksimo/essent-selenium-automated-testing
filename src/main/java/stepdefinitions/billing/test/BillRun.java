...
public class BillRun extends RegisteredScenario {
  ...
  public void i_expect_an_invoice_for_after_date_with_periods(
      String billingId, String invoiceDate, DataTable periods) {
    ...
    Assert.assertEquals(
        "Number of invoice is not correct", 1, invoices.getBillingInvoices().size());

    BillingInvoice billingInvoice = invoices.getBillingInvoices().get(0);

    List<PeriodTable> foundResult = new ArrayList<PeriodTable>();

    for (Entry<Integer, BillingInvoiceOrder> entry :
        billingInvoice.getInvoicedOrders().entrySet()) {
    ...
    for (PeriodTable expected : expectedResult) {
      ...
      Assert.assertTrue(
          "No period found for :" + expected.getStartDate() + " " + expected.getEndDate(), found);
    }
  }

  @Then(
      "^I expect an invoice for \"([^\"]*)\" after date \"([^\"]*)\" for an amount of \"([^\"]*)\"$")
  public void i_expect_an_invoice_for_after_date_for_an_amount_of(
      String billingId, String invoiceDate, String amount) throws Throwable {
    RSGetInvoicesForBillingIDs request = new RSGetInvoicesForBillingIDs();
    DateTime startDate = new DateTime(invoiceDate);
    request.setStartDate(startDate.toDate());
    request.setEndDate(new DateTime("2999-01-01").toDate());
  ...
  @Then(
      "^I expect an invoice for \"([^\"]*)\" after date \"([^\"]*)\" with advance amount change \"([^\"]*)\"$")
  public void i_expect_an_invoice_for_after_date_with_advance_amount_change(
      String billingId, String invoiceDate, String advanceChanged) {
    ...
    Assert.assertEquals(
        "unexpecetd value for advance changed ",
        Boolean.valueOf(advanceChanged),
  ...
  public void i_expect_billing_user_to_have_invoices_after_date(
      String billingId, int number, String invoiceDate) throws Throwable {
    RSGetInvoicesForBillingIDs request = new RSGetInvoicesForBillingIDs();
    DateTime startDate = new DateTime(invoiceDate);
    request.setStartDate(startDate.toDate());
    request.setEndDate(new DateTime("2999-01-01").toDate());
  ...
  public void i_expect_an_entry_in_the_billing_invoice_cache_with_payment_reference(
      String reference) {
    Assert.assertEquals(
        1,
        billingInvoiceCache //
  ...
  @Then(
      "^I expect an invoice for \"([^\"]*)\" after date \"([^\"]*)\" with reactive energy \"([^\"]*)\"$")
  public void i_expect_an_invoice_for_after_date_with_reactive_energy(
      String billingId, String invoiceDate, String amount) {
    RSGetInvoicesForBillingIDs request = new RSGetInvoicesForBillingIDs();
    DateTime startDate = new DateTime(invoiceDate);
    request.setStartDate(startDate.toDate());
    request.setEndDate(new DateTime("2999-01-01").toDate());
    ...
    for (Entry<Integer, BillingInvoiceOrder> entry :
        billingInvoice.getInvoicedOrders().entrySet()) {
      Assert.assertEquals(
          new BigDecimal(amount).setScale(2), entry.getValue().getReactiveEnergy().setScale(2));
    }
  }

  @Then("^I expect an invoice for \"([^\"]*)\" after date \"([^\"]*)\" with kWmax  \"([^\"]*)\"$")
  public void i_expect_an_invoice_for_after_date_with_kWmax(
      String billingId, String invoiceDate, String amount) {
    RSGetInvoicesForBillingIDs request = new RSGetInvoicesForBillingIDs();
    DateTime startDate = new DateTime(invoiceDate);
    request.setStartDate(startDate.toDate());
    ...
    for (Entry<Integer, BillingInvoiceOrder> entry :
        billingInvoice.getInvoicedOrders().entrySet()) {
      Assert.assertEquals(
          new BigDecimal(amount).setScale(2), entry.getValue().getkWmax().setScale(2));
    }
  }
}
