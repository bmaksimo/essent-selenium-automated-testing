...
public class ConsumptionSteps extends DwpScenario {
  ...
  @Autowired private BillingServiceFactory billingServiceFactory;
  ...
  @When(
      "^Consumption at deliverypointid \"([^\"]*)\" with \"([^\"]*)\" hourly-tariff is generated from \"([^\"]*)\" until \"([^\"]*)\" months after$")
  public void generateConsumptionatDeliveryPoint(
      String deliveryPoint, String hourlyTariff, String dateFrom, String months) throws Exception {
    String deliveryPointId = parameterProvider.getValueOrParameterAsString(deliveryPoint);
  ...
  @When(
      "^Consumption at deliverypointid \"([^\"]*)\" is generated from \"([^\"]*)\" until \"([^\"]*)\" months after$")
  public void generateConsumptionUntilRelativeDate(
      String deliveryPoint, String dateFrom, String months) throws Exception {
    String deliveryPointId = parameterProvider.getValueOrParameterAsString(deliveryPoint);
    ...
    String toDate =
        DateTime.parse(fromDate).plusMonths(Integer.parseInt(months)).toString("yyyy-MM-dd");
  ...
  private RestResponse postConsumption(String deliveryPointId, String dateFrom, String dateTo)
      throws Exception {
  ...
  private void postConsumptionTypeElectricityMeterTypeTotalHour(
      String deliveryPointId, String dateFrom, String dateTo) throws Exception {
    ConsumptionRecord consumptionRecord =
        new ConsumptionRecord(deliveryPointId, "Electricity", "TOTAL_HOUR", dateFrom, dateTo);
  ...
  private String getConsumptionRequest(String deliveryPoint, String dateFrom, String dateTo) {
    ...
    String fromDate =
        StringUtils.isBlank(dateFrom) ? DateTime.now().toString("yyyy-MM-dd") : dateFrom;
  ...
  private String getConsumptionRequest(
      String deliveryPoint, String hourlyTariff, String fromDate, String months) {
    UUID uuid = UUID.randomUUID();
    String dateFrom =
        StringUtils.isBlank(fromDate) ? DateTime.now().toString("yyyy-MM-dd") : fromDate;
    String toDate =
        DateTime.parse(dateFrom).plusMonths(Integer.parseInt(months)).toString("yyyy-MM-dd");
