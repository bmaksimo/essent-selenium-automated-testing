...
final class ParameterTransformer implements ParameterByTypeTransformer {
  ...
  ParameterTransformer(Locale locale) {
  ...
  private String getInput(String fromValue) {
    ParameterProvider parameterProvider =
        ((ParameterProvider) ContextService.getContext().getBean("parameterStorage"));
    String result = parameterProvider.getValueOrParameterAsString(fromValue);
    return result;
  }

  @Override
  public Object transform(String input, Type toValueType) {
    String fromValue = getInput(input);
    ...
    if (toValueClass.isEnum()) {
      ...
      throw new CucumberException(
          "Can't transform '" + fromValue + "' to " + toValueType + ". " + "Not an enum constant");
    }
  ...
  private IllegalArgumentException createIllegalArgumentException(
    ...
    return new IllegalArgumentException(
        "Can't transform '"
            ...
            + toValueType
            + "\n"
            + "ParameterTransformer only supports a limited number of class types\n"
            + "Consider using a different object mapper or register a parameter type for "
            + toValueType);
  }
}
