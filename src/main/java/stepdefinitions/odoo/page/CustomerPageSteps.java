...
public class CustomerPageSteps extends OdooScenario {
  ...
  public void odooFormatIs(String format) {
    CustomerPage cp = new CustomerPage();
    Assert.assertThat(
        "Format is not as expected", cp.getDirectDebitFormat().equalsIgnoreCase(format), is(true));
  }
  ...
  public void odooSendToCustomerCheckboxIsChecked() {
    CustomerPage cp = new CustomerPage();
    Assert.assertThat(
        "Sent to customer checkbox is not checked", cp.isSentToCustomerChecked(), is(true));
  }
  ...
  public void odooSendDateIs() {
    CustomerPage cp = new CustomerPage();
    Assert.assertThat(
        "Sent date is not as expected",
        cp.getDirectDebitSentDate()
            .equalsIgnoreCase(LocalDate.now().format(DateTimeFormatter.ofPattern("MM/dd/yyyy"))),
        is(true));
  }
}
