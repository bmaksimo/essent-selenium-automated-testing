...
public class JournalEntriesSteps extends OdooScenario {
  ...
  public void newItemIs(DataTable dbTable) {
    ...
    journalEntriesPage.createNewItem(
        db, 1, parameterProvider.getValueOrParameterAsString(db.get(1).get(1)));
    Sleeper.sleepTightInSeconds(5);
    journalEntriesPage.clickOnAddAnItem();
    journalEntriesPage.createNewItem(
        db, 2, parameterProvider.getValueOrParameterAsString(db.get(2).get(1)));
  }
  ...
  public void reconcileNumberIsRemoved() {
    ExpectedConditions.not(
        ExpectedConditions.visibilityOfElementLocated(
            By.xpath("//td[@data-field='reconcile_id'])[1]/a")));
  }
}
