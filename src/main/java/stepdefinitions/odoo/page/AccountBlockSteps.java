...
public class AccountBlockSteps extends OdooScenario {
  ...
  public void accountBlockStartDateIsToday() {
    String startDate =
        DateExpressionsUtil.getToday().toString(EssentDateTimeFormat.ODOO_DATE_FORMAT.getFormat());
    String pageStartDate = new AccountBlockPage().getAccountBlockStartDate();
    Assert.assertTrue(
        "Account block start date is not today", pageStartDate.equalsIgnoreCase(startDate));
  }
