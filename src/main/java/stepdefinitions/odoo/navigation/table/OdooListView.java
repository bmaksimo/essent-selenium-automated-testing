...
public class OdooListView extends OdooScenario {
  ...
  public void setAdvancedSearchFilter(String expression) {
    String filter =
        parameterProvider.getValueOrParameterAsString(expression) == null
            ? expression
            : parameterProvider.getValueOrParameterAsString(expression);
    String selector = "//div[@class='oe_searchview_input']";
    By xpath = By.xpath(selector);
    List<WebElement> filterElements =
        seleniumDriver.findElements(xpath, Duration.ofSeconds(30), Duration.ofSeconds(5));
  ...
  public void setAdvancedSearchFilter(DataTable dbTable) {
    ...
    AdvancedSearch advancedSearch =
        new AdvancedSearch(list.get(1).get(0), list.get(1).get(1), searchParameter);
  ...
  public void checkCellAt(String column, String ordinal, String value) {
    String input =
        parameterProvider.getValueOrParameterAsString(value) == null
            ? value
            : parameterProvider.getValueOrParameterAsString(value);
