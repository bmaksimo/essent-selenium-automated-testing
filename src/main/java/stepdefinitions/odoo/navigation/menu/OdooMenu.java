...
public class OdooMenu extends OdooScenario {
  ...
  public void clickCodaUrl(String ordinal) {
    ...
    WebElement button =
        seleniumDriver.findElementWhenVisible(
            By.xpath(
                "//table[@class='oe_list_content'][1]//tbody//tr["
                    + rowIndex
                    + "]//td[@data-field='generate_coda']//button[1]"));
  ...
  public void clickButton(String label) {
    WebElement webElement =
        seleniumDriver.findElement(By.xpath("//button//div[contains(., '" + label + "')]"));
  ...
  public void odooContainsModalTitle(String modalTitle) {
    WebElement title =
        seleniumDriver.findElementWhenVisible(By.xpath("//h3[@class='modal-title']"));
    if (null == title || StringUtils.isBlank(title.getText()))
      throw new ExtendedCucumberException("Title was not found");
    assertThat(
        "Current title does not contain " + modalTitle, title.getText().contains(modalTitle));
  }
  ...
  public void odooClickButton(String buttonLabel) {
    ...
    WebElement button =
        seleniumDriver.findElementWhenVisible(
            By.xpath("//button//span[contains(., '" + buttonLabel + "')]"));
    if (null == button)
      throw new ExtendedCucumberException("Button " + buttonLabel + " was not found.");
    button.click();
  }
  ...
  public void odooBankStatementClickButton(String buttonLabel) {
    given()
        ...
        .atMost(new Duration(300, SECONDS))
        .until(() -> inputBankStatementButtonProcessed(buttonLabel));
  }

  private boolean inputBankStatementButtonProcessed(String buttonLabel) {
    WebElement buttonAvailable =
        seleniumDriver.findElement(
            By.xpath(
                "//span[contains(@attrs, 'False')]//button//span[contains(., '"
                    + buttonLabel
                    + "')]"));
  ...
  public void journalEntry() {
    ...
    WebElement journal =
        seleniumDriver.findElement(
            By.xpath(
                "//table[@class='oe_list_content'][1]//tbody//tr[1]//td[@data-field='move_id'][1]//a"));
  ...
  public void modalButtons(String name) {
    awaitOdooRequestToFinish(10);
    WebElement reverseButton =
        seleniumDriver.findElement(By.xpath("//header//button//span[contains(., '" + name + "')]"));
    if (null == reverseButton) throw new ExtendedCucumberException("Button was not found");
    new ButtonImpl(reverseButton).click();

    awaitOdooRequestToFinish(5);
    WebElement reverseModalButton =
        seleniumDriver.findElement(By.xpath("//footer//button//span[contains(., '" + name + "')]"));
  ...
  public void odooValidateBankAccountWasChangedOn(String ban) {
    ...
    CustomerPage cp = new CustomerPage();
    Assert.assertEquals(
        "Check if band account number is same as in DWP",
        cp.getBankAccountAsString(),
        bankAccountNumber);
  }
  ...
  public void odooVerifyPaymentMethodChanged(String pm) {
    ...
    CustomerPage cp = new CustomerPage();
    Assert.assertEquals(
        "Check if payment method is same as in DWP", cp.getPaymentMethodAsString(), paymentMethod);
  }
