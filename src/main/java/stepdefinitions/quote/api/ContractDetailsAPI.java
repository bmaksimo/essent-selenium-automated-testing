...
public class ContractDetailsAPI extends AbstractAPI {
  ...
  private static final Logger LOGGER = Logger.getLogger(ContractDetailsAPI.class);

  public ContractDetails getContractDetails(Cookies cookie, QuoteDetails quoteDetails)
      throws IOException {
    ...
    String contractNumber =
        contractResponse.jsonPath().getString("data.rows[0].cells[3].options.line1");
    contractDetails.setContractNumber(contractNumber);
    LOGGER.debug("Contract number: " + contractNumber);
    String aosProductsId =
        contractResponse.jsonPath().getString("data.rows[0].cells[5].options.params.recordId");
  ...
  public String getPaymentDetails(Cookies cookie, String quoteId, ContractDetails contractDetails)
      throws IOException {
    RequestHelper helper = new RequestHelper();
    String path =
        ConfigProvider.getProperty(ConfigKey.CRM_BASE_URI)
  ...
  public boolean getContractStatus(Cookies cookie, String contractRecordId) throws IOException {
    RequestHelper helper = new RequestHelper();
    String path =
        ConfigProvider.getProperty(ConfigKey.CRM_BASE_URI)
            + ConfigProvider.getProperty(ConfigKey.CRM_CONTRACT_DETAILS_URL)
            + "/"
            + contractRecordId
            + "/"
            + "readOnly";
    ...
    contractStatus =
        contractDetailsResponse.jsonPath().getString("data.model.start_contract_status_c");
  ...
  public boolean getOrderDetails(
    ...
    resultStatusStr =
        getOrderDetailsResponse.xmlPath().getString("//getOrderDetailsResponse/result");
  ...
  public String createXMLPayload4OrderDetails(
      QuoteDetails quoteDetails, ContractDetails contractDetails) throws JsonProcessingException {
