...
public class QuoteBasicFlowB2CSteps extends B2CCreateContractScenario {
  ...
  @When(
      "^Data is prepared for Create quote request for \"([^\"]*)\" and meter open is \"([^\"]*)\" and sign date is \"([^\"]*)\"$")
  public void dataIsPreparedForCreateQuoteWithDateRequestAndPostPreference(
      String flowType, String meterOpen, String signInDate) throws IOException {
    createQuoteAndSaveResult(flowType, meterOpen, signInDate, "POST");
  }

  @When(
      "^Data is prepared for Create quote request for \"([^\"]*)\" and meter open is \"([^\"]*)\" and sign date is \"([^\"]*)\" with communication by email$")
  public void dataIsPreparedForCreateQuoteWithDateRequestAndEmailPreference(
      String flowType, String meterOpen, String signInDate) throws IOException {
    createQuoteAndSaveResult(flowType, meterOpen, signInDate, "EMAIL");
  }

  @When(
      "^Data is prepared for Customer \"([^\"]*)\" and \"([^\"]*)\" and sign date is \"([^\"]*)\"$")
  public void dataIsPreparedForCreateQuotCustomerWallonia(
      String flowType, String area, String signInDate) throws IOException {
    createQuoteCustomerAndSaveResult(flowType, area, signInDate, "POST");
  }

  public void createQuoteAndSaveResult(
      String flowType, String meterOpen, String signInDate, String contactPreference)
      throws IOException {
    this.flow = flowType;
    String signInDateApiDate =
        toDwpAPIDate(parameterProvider.getValueOrParameterAsString(signInDate));
    this.quoteDetails =
        new QuoteDetailsAPI()
            .getQuoteDetails(
                cookie, tariffSheetID, this.flow, meterOpen, signInDateApiDate, contactPreference);
  ...
  public void createQuoteCustomerAndSaveResult(
      String flowType, String area, String signInDate, String contactPreference)
      throws IOException {
    this.flow = flowType;
    String signInDateApiDate =
        toDwpAPIDate(parameterProvider.getValueOrParameterAsString(signInDate));
    this.quoteDetails =
        new QuoteDetailsAPI()
            .getQuoteDetailsCustomer(
                cookie, tariffSheetID, this.flow, area, signInDateApiDate, contactPreference);
    parameterProvider.put("suitecrm-customer", fillInCustomerDetailsContext(this.quoteDetails));
    parameterProvider.put("accountNumber", quoteDetails.getAccountNumber());
    parameterProvider.put("EAN-code", quoteDetails.getEan());
    parameterProvider.put("suitecrm-customer-name", quoteDetails.getAccountName());
  ...
  public void newTcQuoteIsCreated() throws IOException {
    String retrievedQuoteNumber =
        new QuoteDetailsAPI().getQuoteNumber(cookie, quoteDetails.getRecordId());
  ...
  public void contractIsCreated() throws IOException {
    ...
    String contractStartDate =
        new ContractDetailsAPI().getContractDetails(cookie, quoteDetails).getContractStartDate();
    String contractEndDate =
        new ContractDetailsAPI().getContractDetails(cookie, quoteDetails).getContractEndDate();
    ...
    builder
        .append(dayContractDate)
        .append("-")
        .append(monthContractDate)
        .append("-")
        .append(yearContractDate);
    parameterProvider.put("contractDate", builder);
    String retrievedContractNumber =
        new ContractDetailsAPI().getContractDetails(cookie, quoteDetails).getContractNumber();
  ...
  public void paymentDetailsAreReceived() throws IOException {
    this.jbillingId =
        new ContractDetailsAPI()
            .getPaymentDetails(cookie, quoteDetails.getQuoteId(), contractDetails);
    ...
    String path =
        String.format(
            ConfigProvider.getProperty(ConfigKey.CRM_BASE_URI)
                + ConfigProvider.getProperty(ConfigKey.CRM_BATCHJOBSETTING_URL),
            batchJobName,
            targetStatus);
  ...
  public void theBatchjobIsNotRunning(String batchJobName) throws InterruptedException {
    ...
    String path =
        String.format(
            ConfigProvider.getProperty(ConfigKey.CRM_BASE_URI)
                + ConfigProvider.getProperty(ConfigKey.CRM_BATCHJOBSTATE_URL),
            batchJobName);
