...
public class ViewListChecks extends NavigationElements {
  ...
  private static final String TRANSACTIONS_BLOCKED_CHECKMARK =
      "//list[@list-key='TransactionsOnAccount']//td[@class='list__cell cell__text'][${"
          + REPLACEMENT_KEY1
          + "}]//div[@class='customer__status icon-checkmark']";
  ...
  private Map<String, String> getColumnIndexListOptionsFromRow(
      String columnToBeClicked, String tableName, String filterColumn, String data) {
    Integer rowIndex =
        new ViewListTestObject().fetchRowIndexFromData(tableName, filterColumn, data);
    Map<String, String> columnIndexListOptions = new HashMap<>();
  ...
  public void isMatchingValueAtColumn(String tableName, String match, String columnName) {
    ...
    String queryTemplate =
        "//h2[contains(text(), \"%s\")]/parent::div/parent::div/parent::*/parent::div //table[@class=\"list__content\"]/tbody/tr/td[count(//table/thead/tr/th[.=\"%s\"]/preceding-sibling::th)+1]/*[@line-1=\"%s\" or @line-2=\"%s\"]";
  ...
  @Then(
      "^Table \"([^\"]*)\" has matching value \"([^\"]*)\" at column \"([^\"]*)\" polling (\\d+) seconds$")
  public void isMatchingValueAtColumnWithPolling(
      String tableName, String match, String columnName, int waitingTime) {
    ...
    String dashboardMenu =
        parameterProvider.getValueOrParameterAsString("parameter:dashboard-menu");

    FluentWait<ViewListTestObject> waiter = waiter(new ViewListTestObject(), waitingTime, 1);
    String message =
        "Table does not contain cell value "
            + match
            + " at column "
            + columnName
            + " within "
  ...
  public void clickOnViewListAtRowAndColumn(String ordinal, String column) {
    ...
    waiter.until(
        (ClickTableCellUrl callback) -> callback.test(getColumnIndexListOptions(column, ordinal)));
    logger().debug("STEP: " + message + " - PASSED.");
  }

  @When(
      "^Click on link in View List at \"([^\"]*)\" row and \"([^\"]*)\" column polling (\\d+) seconds?$")
  ...
  @When(
      "^Click on link in View List at \"([^\"]*)\" row and \"([^\"]*)\" column waiting for (\\d+) seconds$")
  ...
  @When("^Click on link in \"([^\"]*)\" View List at \"([^\"]*)\" row and \"([^\"]*)\" column$")
  public void clickOnSuppliedViewListAtRowAndColumn(
      String viewListName, String ordinal, String column) {
  ...
  public void listElementWith(String ordinal, String value, String columnName) {
    ...
    waiter.until(
        (ViewListTestObject callback) -> callback.containsDataAt(row, expectedValue, columnName));
    logger().debug("STEP: " + message + " - PASSED.");
  }

  @And(
      "^Table \"([^\"]*)\" contains cell value \"([^\"]*)\" at column \"([^\"]*)\" on \"([^\"]*)\" row$")
  public void listElementWithFromTable(
      String tableName, String value, String columnName, String ordinal) {
    int row = extractNumericValue(ordinal);
    String expectedValue = parameterProvider.getValueOrParameterAsString(value);
    ...
    String message =
        "Table "
            + tableName
            + " did not contain expected cell value "
            + expectedValue
            + " at column "
            + columnName
            + " on "
    ...
    waiter.until(
        (ViewListTestObject callback) ->
            callback.containsCellValue(row, expectedValue, columnName, tableName));
    logger()
        .debug(
            "STEP: Table "
                + tableName
                + " did contains expected cell value "
                + expectedValue
                + " at column "
                + columnName
                + " on "
  ...
  @And(
      "^\"([^\"]*)\" list element has cell value \"([^\"]*)\" at column \"([^\"]*)\" within (\\d+) seconds?$")
  public void containsElementWithin(String ordinal, String value, String columnName, int seconds) {
    ...
    String message =
        ordinal
            + " list element did not contain expected cell value "
            + expectedValue
            + " at column "
            + columnName
            + " within "
  ...
  public void checkInvoice(String value, String column) {
    ...
    while (attempt < maxRetries && !found) {
      try {
        String invoiceIdCellResult =
            new InvoiceListPage()
                .getCellValueOnColumnFromTable(
                    "Transacties",
                    column,
                    filters,
                    parameterProvider.getCurrentContextParameters());
        ...
        logger()
            .warn(parameterProvider.getCurrentContextParameters() + " - Invoice was not found.");
      } finally {
        attempt++;
        if (!found) {
          loopBack(arrow, dashboardMenu);
  ...
  @And(
      "^\"([^\"]*)\" list element has cell value \"([^\"]*)\" at column \"([^\"]*)\" polling (\\d+) seconds?$")
  public void containsElementAt(String ordinal, String value, String columnName, int seconds) {
    ...
    String message =
        ordinal
            + " list element did not have cell value "
            + expectedValue
            + " at column "
            + columnName
            + " within "
  ...
  @And(
      "^\"([^\"]*)\" list element has status \"([^\"]*)\" at column \"([^\"]*)\" within (\\d+) seconds? refreshing \"([^\"]*)\"$")
  public void refreshTillVisible(
      String ordinal, String status, String columnName, int seconds, String linkText) {
    int row = extractNumericValue(ordinal);
  ...
  public void checkDataSelection(String columnName) {
    ...
    assertThat(
        "Data selection at column " + columnName + " is empty", cellSelection, not(hasSize(0)));
    parameterProvider.put(columnName, cellSelection);
    logger()
        .debug(
            "STEP: Cell value(s) from selected row(s) and column "
                + columnName
                + " is/are checked - PASSED.");
  }
  ...
  public void storeColumnValueInSharedProperties(String ordinal, String columnName) {
    ...
    logger()
        .debug(
            "STEP: "
                + ordinal
                + " list element with value at column "
                + columnName
                + " is checked - PASSED.");
  }

  @Then(
      "^\"([^\"]*)\" list element with date interval at column \"([^\"]*)\" from table \"([^\"]*)\" is \"([^\"]*)\"$")
  public void checkDateIntervalTableCell(
      String ordinal, String columnName, String tableName, String interval) {
    int row = extractNumericValue(ordinal);
  ...
  public void storeColumnValueInParameterProvider(
      String match, String columnName, String tableName) {
    seleniumDriver.waitForRequestsToFinish();
    List<String> columnData = new ViewListTestObject().fetchColumnData(tableName, columnName);
    Optional<String> first =
        columnData.stream().filter(element -> element.contains(match)).findAny();
    String message =
        tableName + " list element didn't contain value " + match + " at column " + columnName;
  ...
  public void clickOnElementWithMatchingValue(
      String columnToBeClicked, String match, String filterColumn) {
    new ClickTableCellUrl()
        .test(getColumnIndexListOptionsFromRow(columnToBeClicked, null, filterColumn, match));
  }
  ...
  public void storeColumnValueInParameterProvider(String columnName, String tableName) {
    ...
    List<String> columnData =
        waiter.until(
            (ViewListTestObject callback) -> callback.fetchColumnData(tableName, columnName));
  ...
  public void lookupCellValue(String ordinal, String columnName, String tableName) {
    ...
    String message =
        "Table "
            + tableName
            + " is empty or did not contain any value at "
            + ordinal
            + " row and column "
            + columnName;
  ...
  public void checkSelectionData(String value, String columnName) {
    ...
    waiter.until(
        (ViewListTestObject callback) ->
            CollectionUtils.isNotEmpty(callback.fetchDataSelection(columnName)));
    List<String> cellSelection = viewListModel.fetchDataSelection(columnName);
    ...
    logger()
        .debug(
            "STEP: Selected List rows have cell value "
                + value
                + " at column "
                + columnName
                + " - PASSED.");
  ...
  public void collectViewListElementAsParameter(String viewListElement, String ordinal) {
    ...
    logger()
        .debug(
            "View List element "
                + viewListElement
                + " is collected as parameter at "
                + ordinal
                + " list row");
  }

  @And(
      "^View List element \"([^\"]*)\" using \"([^\"]*)\" as alias is collected as parameter at \"([^\"]*)\" list row$")
  public void collectViewListElementWithAliasAsParameter(
      String viewListElement, String viewListElementAlias, String ordinal) {
    String parameter = getViewListElementAtRow(viewListElement, ordinal);
  ...
  private String getPossibleNumeric(String input) {
    ...
    if (StringUtils.isNumeric(input)) return input;
    else throw new CucumberException(message);
  }
  ...
  private String getViewListElementAtRow(String viewListElement, String ordinal) {
    ...
    assertThat(
        "View List element " + viewListElement + " was not found.",
  ...
  @And(
      "^Table \"([^\"]*)\" contains value \"([^\"]*)\" at column \"([^\"]*)\" within (\\d+) seconds? after clicking on \"([^\"]*)\"$")
  public void viewListContainsValueAtColumn(
      String table, String value, String column, int seconds, String buttonName) {
    String inputValue = parameterProvider.getValueOrParameterAsString(value);
    String dashboardMenu =
        parameterProvider.getValueOrParameterAsString("parameter:dashboard-menu");
  ...
  @And(
      "^Table \"([^\"]*)\" contains value \"([^\"]*)\" at column \"([^\"]*)\" within (\\d+) seconds?$")
  public void viewListContainsValueAtColumn(
      String table, String value, String column, int seconds) {
    String inputValue = parameterProvider.getValueOrParameterAsString(value);
    String arrow = parameterProvider.getValueOrParameterAsString("parameter:navigation");
    String dashboardMenu =
        parameterProvider.getValueOrParameterAsString("parameter:dashboard-menu");
    ...
    waiter.until(
        (ViewListTestObject callback) -> {
          ...
          return CollectionUtils.isNotEmpty(
              callback.fetchColumnData(table, column).stream()
                  .filter(element -> element.contains(inputValue))
                  .collect(Collectors.toList()));
        });
  ...
  @Then(
      "^Table \"([^\"]*)\" contains value \"([^\"]*)\" at column \"([^\"]*)\" retrying (\\d+) times?$")
  public void viewListContainsValueAtColumnRetrying(
      String table, String value, String column, int maxRetries) {
    String inputValue = parameterProvider.getValueOrParameterAsString(value);
    String arrow = parameterProvider.getValueOrParameterAsString("parameter:navigation");
    String dashboardMenu =
        parameterProvider.getValueOrParameterAsString("parameter:dashboard-menu");
    ...
    while (attempt < maxRetries && !found) {
      ...
      found =
          CollectionUtils.isNotEmpty(
              new ViewListTestObject()
                  .fetchColumnData(table, column).stream()
                      .filter(element -> element.contains(inputValue))
                      .collect(Collectors.toList()));
    ...
    logger()
        .debug(
            value + " was found in " + table + " at " + column + " column at attempt #" + attempt);
  }
  ...
  public void viewListContainsValueAtColumn(String table, String value, String column) {
    ...
    List<String> found =
        columnData.stream()
            ...
            .collect(Collectors.toList());
    String message =
        "Table " + table + " didn't contain value " + inputValue + " at column " + column;
  ...
  public void viewListContainsCheckmarkAtColumn(String table, String column) {
    ...
    logger()
        .debug("STEP: Table " + table + " contains check mark at column " + column + " - PASSED.");
  }

  @And(
      "^Table \"([^\"]*)\" does not contain value \"([^\"]*)\" at column \"([^\"]*)\" within (\\d+) seconds?$")
  public void viewListDoesNotContainsValueAtColumn(
      String table, String value, String column, int seconds) {
    String arrow = parameterProvider.getValueOrParameterAsString("parameter:navigation");
    String dashboardMenu =
        parameterProvider.getValueOrParameterAsString("parameter:dashboard-menu");
    ...
    String message =
        "Table " + table + " should not contain value " + value + " at column " + column;
    assertThat(message, found, empty());
  }

  @And(
      "^Table \"([^\"]*)\" does not contain any value at column \"([^\"]*)\" within (\\d+) seconds?$")
  ...
  @And(
      "^\"([^\"]*)\" element of table \"([^\"]*)\" at currency column \"([^\"]*)\" is the sum of contracts amounts$")
  public void checkCurrencyAmountDableDataAsSum(String ordinal, String table, String columnName) {
    seleniumDriver.waitForRequestsToFinish();
    String stringSumOfContracts =
        parameterProvider.getValueOrParameterAsString("parameter:sum-of-contracts");
    int sumOfContracts = Integer.parseInt(stringSumOfContracts);

    int row = extractNumericValue(ordinal);
    Optional<String> currencyValue =
        new ViewListTestObject().getCurrencyValueAt(row, columnName, table);
    String message = ordinal + " list element didn't contain any value at column " + columnName;
    assertThat(message, currencyValue.isPresent(), is(true));
    int actualAmount = amountInCurrencyAsInt(currencyValue.get());
    assertThat(
        "Total advance prepaid amount %s is not equal to sum of sub amounts %s",
        actualAmount, equalTo(sumOfContracts));
  }

  @And("^\"([^\"]*)\" in the first \"([^\"]*)\" row of \"([^\"]*)\" table is \"([^\"]*)\"$")
  public void checkCurrencyAmount(
      String columnToSearch, String optionToSearch, String table, String expression) {
    seleniumDriver.waitForRequestsToFinish();
    Optional<String> currencyValue =
        new ViewListTestObject().getCurrencyValueAt(1, columnToSearch, table);
  ...
  public void checkInvoiceCurrencyAmount(String expectedCurrencyAmountAsString) {
    ...
    String message =
        "Table Transactions doesn't have value for invoice outstanding amount of "
  ...
  public void checkInvoiceAmount() {
    ...
    Long unsignedCreditInvoice =
        Long.parseLong(actualValuesOfInvoices.get("creditInvoice").substring(1));
  ...
  public void checkValue() {
    ...
    String expectedBalance =
        parameterProvider.getValueOrParameterAsString("parameter:additionalInvoice") + ",00";
    ...
    switch (typeRate) {
      ...
      default:
        throw new CucumberException(
            getClass() + ": Only High and Low values can be passed as parameters");
    }
  }
  ...
  public void compareSumOfRatesForSignatureQuoteAndRejectedQuote(
      String sumRatesHighSignature, String sumRatesLowSignature) {
    ContractPage cp = new ContractPage();
    float sumHighRatesSignatureToFloat =
        Float.parseFloat(parameterProvider.getValueOrParameterAsString(sumRatesHighSignature));
    float sumLowRatesSignatureToFloat =
        Float.parseFloat(parameterProvider.getValueOrParameterAsString(sumRatesLowSignature));
    cp.sumRates(sumRatesHighSignature);
    cp.sumRates(sumRatesLowSignature);
    assertThat(
        "Sum of signature and rejected quote for High prices are not equal",
        sumHighRatesSignatureToFloat,
        equalTo(cp.sumRates(sumRatesHighSignature)));
    assertThat(
        "Sum of signature and rejected quote for Low prices are not equal",
        sumLowRatesSignatureToFloat,
        equalTo(cp.sumRates(sumRatesLowSignature)));
  }
    ...
    while (attempt < maxRetries && !found) {
      try {
        List<WebElement> results =
            new MarketMessagesPage()
                .selectRowOnTable(
                    dataTableFilters.get(1).get(0),
                    filters,
                    parameterProvider.getCurrentContextParameters());
