...
public class ContractsSteps extends DwpScenario {
  ...
  public void paymentDelayed() {
    InvoiceListPage ilp = new InvoiceListPage();
    ilp.checkPayDate(
        parameterProvider.getValueOrParameterAsString("parameter:Datum & Vervaldatum"));
  }
  ...
  public void compareStartAndEndDate(String startDate, String ex, String endDate) {
    ...
    Assert.assertTrue(
        String.format(
            "Start date \"%s\" differs from the end date \"%s\" by more than 1 year ", sd, ed),
        Arrays.asList(expectedRange).contains(ar));
  }
  ...
  public void checkTableValueMatches(String tableValue) {
    ...
    assertThat(
        String.format("Table value \"%s\" does not exist in quote data", tableValue),
        expectedValue,
        is(true));
  }
  ...
  public void calculateAmountInstallment(String invoiceAmount) {
    ...
    int formedAmountPerInstallment =
        new ContractPage().getAmountPerInstallment(invoice, firstInstallment);

    assertTrue(
        String.format(
            "Amount per installment \"%s\" is not at least 50", formedAmountPerInstallment),
        formedAmountPerInstallment >= 50);
  ...
  public void checkInstallmentsNumber(int expectedNumberOfInstallments, String amount) {
    ...
    assertTrue(
        "Insufficient Number of installments with given amount.",
        expectedNumberOfInstallments <= actualNumberOfInstallments);
  }
