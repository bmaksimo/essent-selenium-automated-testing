...
public class QuoteSteps extends DwpScenario {
  ...
  public void deduplicationDialogueIsShown(String title) {
    assertThat(
        "Similar clients dialogue was not shown.",
        new SimilarAccountDialogImpl(title).getTitle(),
        equalTo(title));
  }
  ...
  public void duplicateRandomUser() {
    CustomerDetails customerDetails =
        (CustomerDetails) parameterProvider.getValueOrParameter("parameter:suitecrm-customer");
  ...
  public void checkValuesGreaterThanZero(String label) {
    Assert.assertTrue(
        "Not all values are greater than zero",
        new NewQuotePage().areAllAmountsGreaterThanZero(label));
  }
  ...
  public void onlineSignQuote() throws Throwable {
    ...
    assertThat(
        ...
        is(document.exists()));
    SignatureData signature = new SignatureData(DwpDateFormats.DWP_TODAY, path);
    OnlineQuoteSignatureModalPage onlineQuoteSignatureModalPage =
        new OnlineQuoteSignatureModalPage();
