...
public class MarketBerichtenSteps extends NavigationElements {
  ...
  public void refreshTillIsVisible(String name, String status) {
    ...
    waiter.withMessage(
        String.format(
            "Message status did not switch to \"%s\" within \"%s\" seconds",
            status, atMostSeconds));
    waiter.until(
        (MarketMessagesPage page) -> {
  ...
  public void marketBerichtWithEANAndModuleSecondTransactionIsInStatus(
      String modul, String status) {
    MarketMessagesPage mp = new MarketMessagesPage();
  ...
  public void checkMarketMessage(final DataTable dbTable) {
    ...
    for (int i = 0; i <= dataTableFilters.size(); i++) {
      ...
      currentColumnValue =
          currentColumnName.toUpperCase().contains("& ED")
      ...
      try {
        List<WebElement> results =
            new MarketMessagesPage()
                .selectRowOnTable(
                    "Marktberichten", filters, parameterProvider.getCurrentContextParameters());
        found = CollectionUtils.isNotEmpty(results);
      } catch (Exception e) {
        logger()
            .warn(
                parameterProvider.getCurrentContextParameters()
                    + " - Market message was not found.");
      } finally {
        attempt++;
        if (!found) {
  ...
  public void checkMarktbericht(final DataTable dbTable) {
    ...
    if (ean.equalsIgnoreCase(mp.getEan(eanFromMarketberichtRow1))
        && (mp.getModule(modulFromMarketberichtRow1).equals(modul))) {
      Assert.assertEquals(modul, mp.getModule(modulFromMarketberichtRow1));
      Assert.assertEquals(
          mp.getEndDate(marketberichtEndDateElementRow1),
          toDwpEndDate(parameterProvider.getValueOrParameterAsString(date)));
    } else {
      if (ean.equalsIgnoreCase(mp.getEan(eanFromMarketberichtRow2))
          && (mp.getModule(modulFromMarketberichtRow2).equals(modul))) {
        Assert.assertEquals(modul, mp.getModule(modulFromMarketberichtRow2));
        Assert.assertEquals(
            mp.getEndDate(marketberichtEndDateElementRow2),
            toDwpEndDate(parameterProvider.getValueOrParameterAsString(date)));
      } else {
        if (ean.equalsIgnoreCase(mp.getEan(eanFromMarketberichtRow3))
            && (mp.getModule(modulFromMarketberichtRow3).equals(modul))) {
          Assert.assertEquals(modul, mp.getModule(modulFromMarketberichtRow3));
          Assert.assertEquals(
              mp.getEndDate(marketberichtEndDateElementRow3),
              toDwpEndDate(parameterProvider.getValueOrParameterAsString(date)));
        }
      }
    }
