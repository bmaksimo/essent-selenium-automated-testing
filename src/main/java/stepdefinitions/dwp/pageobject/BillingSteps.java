...
public class BillingSteps extends DwpScenario {
  ...
  public void transactiesIsCreatedWithTYPE(String type) {
    Assert.assertTrue(
        "Type does not mach", new BillingPage().selectProductCode().equalsIgnoreCase(type));
  }
  ...
  public void cnwAndVKWInvoiceHaveSameInvoiceAmount() {
    ...
    Assert.assertTrue(
        String.format(
            "cnw \"%s\" and vkw \"%s\" does not have the same invice amount", amountCNW, amountVKW),
        amountCNW.equalsIgnoreCase(amountVKW));
  }
}
