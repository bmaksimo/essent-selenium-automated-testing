...
public class PaymentInfoSteps extends NavigationElements {
  ...
  public void listSwitchedPaymentMethod() {
    final By wireTramsferPaymentMethod =
        By.xpath("//list-simple-two-liner-cell[contains(@line-2,'Overschrijving')]");
    WebElement element = seleniumDriver.findElementWhenPresent(wireTramsferPaymentMethod);
    assertThat(
        String.format("View list did not contain payment method %s", "Overschrijving"),
        element,
        is(notNullValue()));
  }
