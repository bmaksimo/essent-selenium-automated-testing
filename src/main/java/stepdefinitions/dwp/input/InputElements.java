...
public class InputElements extends DwpScenario {
  ...
  @And(
      "Search for \"([^\"]*)\" in the SelectWithSearch \"([^\"]*)\" and select option \"([^\"]*)\" and Submit")
  public void searchForInTheSelectWithSearchAndSelectOptionAndSubmit(
      String searchText, String selectWithSearchLink, String optionToSelect) {
    // Grab and click the base element
    seleniumDriver
        .findElementWhenClickable(
            By.xpath(
    ...
    seleniumDriver.waitAndClick(
        seleniumDriver.findElementWhenClickable(
            By.xpath(
                String.format(
                    "//div[@class='multi-select__results']/ul[@class='jq-non-selected-results']//label[text()=' %s ']",
                    optionToSelectParsed))));

    // Aaaaaaand submit ;-)
    seleniumDriver.waitAndClick(
        seleniumDriver.findElement(By.xpath("//a[normalize-space(text())='Verzenden']")));
    seleniumDriver.waitForRequestsToFinish();
  }
  ...
  public void setDateInputFirstDayNextMonth(String label) {
    String value =
        DateExpressionsUtil.getFirstDateOfNextMonth()
  ...
  public void setDateInputLastDayCurrentMonthNextYear(String label) {
    String value =
        DateExpressionsUtil.getLastDayOfCurrentMonthNextYear()
            .toString(EssentDateTimeFormat.DWP_FRENCH_DATE_FORMAT.getFormat());
    setDateInput(label, value);
  }
  ...
  private void setValidToTime(String time) {
    ...
    seleniumDriver.waitAndSendKeys(
        seleniumDriver.findElement(By.id(CALENDAR_VALIDTO_TIME_ID)), validTo);
  }
  ...
  public void setOption(String label, SwitchState state) {
    ...
    List<WebElement> inputList =
        seleniumDriver.findElements(By.xpath("//div[label=\"" + label + "\"]//input"));
  ...
  public WebElement getFieldUntilPresent(String placeholder) {
    ...
    while (!elementFound && currentAttempt <= maxAttempts) {
      currentAttempt++;
      Optional<WebElement> placeholderElementOptional =
