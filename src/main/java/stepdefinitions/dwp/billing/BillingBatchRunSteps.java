...
public class BillingBatchRunSteps extends DwpScenario {
  ...
  public void startBillingRun(String jobName, String processDate) {
    ...
    SimpleDateFormat formatter =
        new SimpleDateFormat(EssentDateTimeFormat.DWP_API_DATE_FORMAT.getFormat());
    ...
    while (!success && currentAttempt <= maxRetries) {
      try {
        BillingRunResult result =
            billingService.startBillRun(jobName, billingId, formatter.parse(inputValue));
        currentAttempt++;
  ...
  public void startMediation(String jobName, String settlementDate) {
    ...
    try {
      ...
      MediationRunResult mediationRunResult =
          billingService.runMediationJob(jobName, billingCustomerId, deliveryPointId, parsedDate);
