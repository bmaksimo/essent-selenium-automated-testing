...
import com.essent.testing.dwp.pageobject.salesmarketing.focusedmode.PricingToolDashboardPage;
...
public class PricingToolDashboardSteps extends DwpScenario {
  ...
  public void tablePriceCalculationIsNotEmpty() {
    ...
    Assert.assertTrue(
        "Purchase price column is not displayed", ptdp.getPurchasePriceHeader().isDisplayed());
    Assert.assertTrue(
        "Purchase price column first row is not displayed",
        ptdp.getPurchasePriceFirstRow().isDisplayed());
  }
