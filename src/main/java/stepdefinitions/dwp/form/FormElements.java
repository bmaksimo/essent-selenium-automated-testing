...
public class FormElements extends DwpScenario {
  ...
  public void checkFormHeader(String expectedFormHeader) {
    ...
    WebElement actualFormHeader =
        seleniumDriver.findElementWhenVisible(
            By.xpath(
                "//div[contains(@class, 'form__header')]/*[normalize-space()='"
                    + expectedFormHeader
                    + "']"));
    assertThat(
        "Form header " + expectedFormHeader + " was not displayed within given time.",
        actualFormHeader.isDisplayed());
  }
  ...
  public void checkValueInCard(String label, String cardName, String expectedParameter) {
    ...
    waiter.until(
        field -> StringUtils.equalsIgnoreCase(field.getValue(cardName, label), expectedValue));
  }
