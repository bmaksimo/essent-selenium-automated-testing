...
public class DetailsFormSteps extends DwpScenario {
  ...
  public void checkFieldValue(String label) {
    ...
    assertThat(
        String.format("'%s' field was empty, was expected to have value", label),
  ...
  public void customerTypeIs(String type) {
    ...
    Assert.assertTrue(
        "Customer type does not match expected value", dp.getCustomerType().equalsIgnoreCase(type));
  }
  ...
  public void customerBankNumberIsAndPaymentMethodIs(String iban, String method) {
    DetailsPage dp = new DetailsPage();
    Assert.assertThat(
        "Customer iban does not match expected value",
        dp.getIban().equalsIgnoreCase(parameterProvider.getValueOrParameterAsString(iban)),
        is(true));
    Assert.assertThat(
        "Customer payment method does not match expected value",
        dp.getPaymentMethod().equalsIgnoreCase(method),
        is(true));
  }
  ...
  public void accountBlockStartDateIsToday() {
    String startDate =
        DateExpressionsUtil.getToday()
            .toString(EssentDateTimeFormat.DWP_BILLING_DATE_FORMAT.getFormat());
    DetailsPage dp = new DetailsPage();
    Assert.assertThat(
        "Account block start date is not today",
        dp.getAccountBlockStartDate().equalsIgnoreCase(startDate),
        is(true));
  }
  ...
  public void accountBlockEndDateIsToday(int amountOfDays) {
    String endDate =
        DateExpressionsUtil.getNDaysFromToday(amountOfDays)
            .toString(EssentDateTimeFormat.DWP_BILLING_DATE_FORMAT.getFormat());
    String pageEndDate = new DetailsPage().getAccountBlockEndDate();
    Assert.assertThat(
        "Account block end date is not " + amountOfDays + " days from today",
        pageEndDate.equalsIgnoreCase(endDate),
        is(true));
  }
  ...
  public void accountBlockEndDateNotExist() {
    DetailsPage dp = new DetailsPage();
    Assert.assertNull(
        "Account block end date exists" + dp.getAccountBlockEndDateNotExist(),
        dp.getAccountBlockEndDateNotExist());
  }
  ...
  public void accountBlockStartAndEndDatesAreTheSame() {
    ...
    Assert.assertThat(
        "Dunning account block star and end dates are different",
        dp.getAccountBlockStartDate().equalsIgnoreCase(dp.getAccountBlockEndDate()),
        is(true));
  }
